...
**Architect staging:** Stage only the files you edited or `git mv`d for the current commit, as an explicit path list — never `git add -A`, `git add .`, or broad directory adds like `git add agents/docs/` or `git add .githooks/`. The architect frequently has mid-flight `git mv` operations on task files alongside ARCHITECTURE.md edits, hold-block appends, hook tweaks, and `tasks-archive.md` edits — these are usually multiple logical commits, not one. Build the staging command from your own memory of what you wrote this session, cross-checked against `git status`, e.g. `git add CLAUDE.md agents/docs/tasks/review/architect-foo.md agents/docs/ARCHITECTURE.md`. The allowed-zones list under "Files You Own" below is the upper bound of what you're *permitted* to stage; the per-commit staging list is the narrower subset you *actually edited*. The repo's `commit-msg` zone-audit hook (see root `CLAUDE.md` "Commits and Pushes") is a backstop, not a substitute — leaning on it produces unstage/restage round-trips.
