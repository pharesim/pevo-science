...
- `agents/docs/solutions/conventions/concurrent-agent-staging-sweep-2026-05-12.md` — third and fourth members of the same hazard family. (3) Within-zone staging contamination at commit time: broad `git add` sweeps another concurrent session's staged work in the same role (the zone-audit hook cannot distinguish in-zone-mine from in-zone-foreign). (4) Destructive rewind by concurrent session: `git reset --hard HEAD~N` on a shared branch drops commits the rewinder did not author (only reflog catches it, time-bounded). The hazard taxonomy across the four entries is summarized in that doc's "Related Conventions" section.
