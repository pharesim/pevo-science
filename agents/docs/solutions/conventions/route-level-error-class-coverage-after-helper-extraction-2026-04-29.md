...
- `vitest-tdz-factory-closure-vs-import-graph-eager-eval-2026-05-16.md` — disambiguates this doc's `vi.hoisted + await import` pattern (factory-closure TDZ) from a sibling shape (import-graph eager-evaluation TDZ) where module-scope `await import` is the right fix instead. If your TDZ doesn't match the factory-closure shape this doc covers, check there before reaching for `vi.hoisted`.
