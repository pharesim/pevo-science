...
- [wrapping-primitive-exhaustive-call-site-audit-2026-04-22.md](wrapping-primitive-exhaustive-call-site-audit-2026-04-22.md) — complementary on the null-path axis. That doc's rule: TypeScript SQL result types must permit `null` where null is possible, so the compiler forces sub-branch handling at the call site. Without it, a non-nullable annotation hides the null sub-branch from both the compiler and from a reviewer applying this doc's sub-branch enumeration checklist. The `resume-signup` ORCID-only null-hash case (TypeError in ~0ms) is one instance of this doc's rule surfaced by that doc's annotation fix.
