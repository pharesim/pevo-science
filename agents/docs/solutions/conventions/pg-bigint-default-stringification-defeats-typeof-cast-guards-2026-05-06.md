...
- `agents/docs/solutions/conventions/pg-typed-query-generic-compile-time-only-not-sql-projection-guard-2026-06-09.md` — sibling on a different axis. There, the trap is COLUMN PRESENCE (a field dropped from the `SELECT` is not caught by the `pool.query<T>` generic, which is compile-time-only); here, the trap is VALUE coercion (a `bigint` arrives as a JS string regardless of the cast). Same root: pg/TS typing is not a runtime SQL contract.
