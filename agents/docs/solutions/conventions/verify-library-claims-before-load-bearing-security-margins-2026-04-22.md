...
- `agents/docs/solutions/conventions/verify-resource-knob-math-before-load-bearing-security-margins-2026-04-22.md` — sibling: unverified **runtime-knob math** (UV_THREADPOOL_SIZE vs argon2 parallelism) cascading through a security margin. Same "verify before load-bearing" family applied to resource sizing instead of library behavior.
- `agents/docs/solutions/conventions/chain-write-timeout-ambiguous-outcome-2026-04-22.md` — **consequential companion.** The dhive-timeout cascade motivated adding `broadcastJsonWithTimeout`; that fix closed the library-claim gap but opened a new one — a timer fire doesn't distinguish "broadcast accepted but response hung" from "broadcast never reached node." When integrating an application-owned timeout following this doc's guidance, also read the companion before writing the error-handling path.
