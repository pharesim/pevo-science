...
- [[universal-mock-inventory-must-be-re-derived-not-incrementally-patched-2026-06-11]] — the third audit in the family: that doc covers falseness SURVIVING in the unchanged remainder of an edited universal claim (only/all/every inventories must be re-derived from ground truth, not delta-patched); this doc covers rot the fix's ADDED text introduces. Run both when a fix edits a universal claim and adds code.
