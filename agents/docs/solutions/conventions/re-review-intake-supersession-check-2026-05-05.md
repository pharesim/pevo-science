...
- **Different artifact, same intake checkpoint:** [`re-review-intake-stale-gated-annotation-2026-05-26.md`](./re-review-intake-stale-gated-annotation-2026-05-26.md). The three dimensions above are all SHA-trust (they verify a commit claim). That convention adds an annotation-trust axis: a "gated/blocked-on-X" deferral annotation verified against code, because the gate can ship while the gating task lags in `pending/`.
- **Content-match dimension for held-item classes:** [`re-review-intake-green-suite-not-held-item-completion-2026-06-09.md`](./re-review-intake-green-suite-not-held-item-completion-2026-06-09.md). The three SHA-trust dimensions above verify that a commit is reachable and its diff scope survived at HEAD; that convention verifies that each *held item* actually landed inside the commit. Four item classes (type-derivation, log-field rename, add-a-test, single-layer-shared abstraction) pass typecheck/lint/tests whether or not they landed, so a green gate cannot confirm them — a per-item diff audit can.
