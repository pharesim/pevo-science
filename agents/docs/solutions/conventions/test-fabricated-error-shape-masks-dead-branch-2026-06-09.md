...
- `inner-catch-shadows-outer-catch-in-route-tests-2026-04-28.md` — sibling in the same "vacuous error-handling test" family, a different mechanism: there the branch is shadowed (or dead) because a closer/nested handler (an inner catch, or a recursive call's own outer catch) intercepts the simulated error before it reaches the branch under test; here the branch is dead because the guard reads a field the real error class never sets.
