...
When the staging command returns an unexpected staged set (extra paths you did not touch), prefer `git restore --staged <path>` over `git reset HEAD <path>`. `git reset HEAD` with explicit paths can collateral-unstage adjacent index entries — verified 2026-05-15: a `git reset HEAD <sibling-rename-paths>` unstaged the orchestrator's own edits as well. `git restore --staged` is path-scoped *in most cases* — but NOT when git's content-based rename detection has grouped your staged change with a sibling's. Verified 2026-05-20: `git restore --staged <sibling-rename-target>` silently unstaged unrelated `git rm` deletions whose deleted-file content matched the sibling-rename target above the default 50% similarity threshold. After any `git restore --staged` in a shared checkout, re-verify with `git diff --cached --stat` that nothing else was collateral-unstaged. See `git-restore-staged-can-unstage-grouped-rename-detected-deletions-2026-05-20.md` for the rename-detection collateral-unstage variant.
