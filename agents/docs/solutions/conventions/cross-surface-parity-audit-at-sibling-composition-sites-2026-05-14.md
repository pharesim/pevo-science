...
5. **Name the axis of parity, not just "match the canonical sibling."** Sibling composition sites may legitimately diverge on dimensions that are NOT part of the named invariant. Concrete case from the parity-gate task (2026-05-15): the 4 newly-gated sites share the parent-paper class gate uniformly, but `search.ts` review search intentionally omits the anon-proxy OR-arm (`OR c.author = $anonAccount`) that the other 3 sites and `reputation.ts` include — because list search has no need to surface anon-proxy reviews, and including them would partially de-anonymize the proxy mapping. This is NOT a parity violation: the invariant names parent-paper class admission, not author admission; the dimensions are independent. But the divergence must be (a) documented as an in-source comment at the diverging site cross-referencing the canonical sibling, AND (b) recorded in the enumeration's per-site disposition as "match on `<named axis>`, diverge on `<other axis>` because `<reason>`." Otherwise a future contributor reading the convention doc sees the 4-site enumeration, assumes uniform composition, "fixes" search.ts to match reputation.ts on author admission, and accidentally surfaces anon-proxy reviews — a privacy regression introduced by a well-intentioned parity edit. Composition-shape parity is a stricter claim than invariant parity; conflating them produces a different failure mode than the one the audit was protecting against.
