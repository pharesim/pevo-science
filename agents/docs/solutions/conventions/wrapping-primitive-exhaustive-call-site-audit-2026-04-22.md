...
11. Any `/ce-code-review` finding that names a specific code reference (file path + symbol name + line number) at any confidence level — including cross-reviewer corroborated findings at confidence 100: the architect MUST verify the reference exists before acting on the finding. A 2-second `grep <symbol> <path>` settles it. Cross-reviewer agreement amplifies confidence in real findings, but it can also amplify shared hallucinations when reviewers latch onto the same plausible-but-wrong inference (e.g., extrapolating from a task body's prior-state references to claim those references are still in the current code). The "claim, not evidence" principle applies symmetrically: an implementer's signal block listing sites is a claim, AND a reviewer's finding citing sites is a claim. Both need the grep. Architect's verification step before any state change (filing a fix task, opening a hold block, archiving on a finding-resolved basis) takes seconds; acting without verification on a hallucinated reference produces phantom work and erodes the review pipeline's signal-to-noise ratio. Surfaced 2026-04-30 during cluster 3 R3+R4 review when both `ce-testing-reviewer` and `ce-correctness-reviewer` independently flagged "orphaned `poolQueryMock` tests in `account-creation.test.ts`" at confidence 100 each; verification by grep returned zero matches — the rewritten test file was clean and both reviewers had confabulated the reference, probably from the original task body's pre-rewrite code that the migration correctly removed.
12. Any architect hold block names a grep audit as a required check on a multi-call-site primitive: tag the grep as **`implementer-MUST-at-signal-block`**, not `architect-followup-AT-ARCHIVE`. The grep runs at the implementer's signal-block-write boundary (the moment before `git mv pending/ → review/` after completing a round), with the output included verbatim in the signal block. Deferring the grep to architect re-review intake or archive reproduces the gap one round later — round-N catches what round-(N-1) missed, but only if round-N's signal block already ran the audit; otherwise the catch slips to round-(N+1) and the task accumulates an extra hold-fix-re-review cycle per deferred audit item. Rule 7 (re-review must include the grep) is the architect-side default — a confirming scan, not a discovery scan; this rule is the implementer-side default that prevents reaching architect re-review with the discovery work still pending. See [`load-bearing-greps-at-signal-block-write-time-2026-05-06.md`](load-bearing-greps-at-signal-block-write-time-2026-05-06.md) for the canonical-walker round-2 → round-3 evidence (architect deferred the grep audit to "before archive"; implementer ran it at signal-block-write-time anyway and discovered a fourth call site at `papers.ts:571` that round-2's hold-block enumeration had missed) and the corresponding hold-block phrasing template architects should use. Surfaced 2026-05-06 during canonical-walker round-3 archive.
...
- [load-bearing-greps-at-signal-block-write-time-2026-05-06.md](load-bearing-greps-at-signal-block-write-time-2026-05-06.md) — temporal-axis sibling. This doc establishes "grep don't enumerate mentally" but is silent on WHEN in the round protocol the grep runs; the sibling fills the gap with the implementer-at-signal-block-write-time rule. Rule 12 above forward-references it. Read together: this doc says WHETHER to grep and how to phrase the audit; the sibling says WHEN in the implementer-architect handoff cycle the grep must fire. Both are necessary; deferring either reproduces the failure mode this doc was written to prevent.
- [strict-superset-wrapper-inherits-escape-hatches-2026-05-12.md](strict-superset-wrapper-inherits-escape-hatches-2026-05-12.md) — DUAL axis. This doc is about the call-site coverage axis (every call-site of the underlying class must route through the wrapper; completeness pulls the wrapper surface OPEN). The sibling is about the option-surface axis (when completing the superset, the wrapper inherits the wrapped library's policy-override escape hatches; the safety contract pulls the surface CLOSED). The two rules pull in opposite directions and force per-option triage at wrapper-extension time — completing the call-site coverage is necessary but not sufficient for wrapper integrity when the wrapper layers a safety contract.
- [helper-contract-flip-untouched-adopter-audit-2026-05-16.md](helper-contract-flip-untouched-adopter-audit-2026-05-16.md) — INVERSE direction. This doc audits at helper introduction: "did all sites adopt the helper?" The sibling audits at helper semantic change: "do all already-adopted sites still hold against the helper's new contract?" Rule 12's narrow form (error-class addition forces re-audit of every existing call site) is the same shape as the sibling's generalized rule (any internal defaulting-semantics flip forces re-grade of every existing adopter). Run both — this doc at helper introduction, the sibling at every later semantics change.
