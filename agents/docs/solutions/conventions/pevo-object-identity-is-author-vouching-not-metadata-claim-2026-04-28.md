...
- JS/TS: branches on metadata-derived discriminants followed by privilege grants.
  ...
  - Pattern: `pevo.continues = { author, permlink }` (continuation pointer admitted into a paper's version chain) — concrete second instance of this anti-pattern after `bridge_paper`. Predicate shape differs: continuation authorization is **set membership** in the paper's authorized-author set (a vouched-identity set scoped per-resource), not equality to a single pinned account. The membership set is the **cumulative chain `authors[]`** — the append-only union of `pevo.authors[].hive` across every admitted chain post (per `ARCHITECTURE.md § 2 "Display construction (cumulative union)"`), not the root post's `authors[]` alone. The structural rule is unchanged by that shift: the OR-arm must terminate in an identity predicate, and the predicate shape can be `author === <pinned>` OR `author IN <vouched-set>`; only the *contents* of `<vouched-set>` widened from "root's authors" to "cumulative-union authors" so a co-author added mid-chain can continue. See `agents/docs/solutions/architecture-patterns/pevo-paper-version-chain-and-edit-semantics-2026-04-30.md` for the version-chain semantics this gate operates on.
...
- [`hive-primitive-aware-design-rules-for-pevo-custom-json-ops-2026-05-05.md`](hive-primitive-aware-design-rules-for-pevo-custom-json-ops-2026-05-05.md) — same "self-assertion is not authorization" reasoning extended to the custom_json op-type axis. That doc's rule 5 (signer-binding for consent ops) is the op-type analog of this doc's gate-predicate rule; the two together cover the "identity must be a structural predicate" pattern across SQL gates and on-chain consent ops.
