...
- `agents/docs/solutions/conventions/re-review-intake-stale-gated-annotation-2026-05-26.md` — mirror at the same intake checkpoint, opposite failure direction. This convention verifies a claimed-*landed* SHA is reachable from main; that one verifies a claimed-*still-gated* deferral annotation against code (the gate may have shipped while the gating task lags in `pending/`). Both: coordination artifacts are not authoritative, verify against code/git.
