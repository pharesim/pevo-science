...
Auth/permission middleware (`verifyHiveSignature`) is real-by-default and MUST run real in any test whose focus IS authentication or whose assertions depend on cryptographic-verification behavior. One narrow exception was added to root `CLAUDE.md` clause (b) after this doc's original lead-in: for tests whose focus is downstream behavior (SQL shape, response envelope, route plumbing) and NOT cryptographic verification, the project-wide `MOCK_VERIFY_SIGNATURE` fixture (`backend/tests/fixtures/mock-auth.ts`) is acceptable. The fixture preserves the 401-on-missing-header gate and the username-extraction behavior; only the cryptographic signature check is bypassed. When it is used, the test header must acknowledge it under clause (a) (state that crypto verification is bypassed and why the focus permits it), and a clause-(c) real-path companion MUST exercise the real `verifyHiveSignature` middleware against signed requests for the same or a sibling route. The distinction is the test's *focus*, not the act of mocking: an auth-focused or crypto-dependent test mocking the middleware is still a clause-(b) violation regardless of any companion.
...
A test whose focus IS authentication, or whose assertions depend on cryptographic-verification behavior, mocking `verifyHiveSignature` to skip the signature check. Clause (b) excludes this regardless of real-path companions: such a test must run the real middleware. (Contrast: a test whose focus is downstream behavior and NOT crypto verification may mock via the `MOCK_VERIFY_SIGNATURE` fixture per clause (b)'s refinement — that is a clause-(b)-compliant use with a clause-(a) header + a clause-(c) real-`verifyHiveSignature` companion, not this non-compliant case. The line is the test's focus, not the act of mocking.)
