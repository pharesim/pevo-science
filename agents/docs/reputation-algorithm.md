...
> **Implementation status.** The consented-set credit model above is **live**. `computeReputationBatch` credits the union Route 1 (root broadcaster) ∪ Route 2 (`author_accept` on a hive- or attested-ORCID-anchored slot, resolved by the shared `consentChainCteBody` + `consentedAuthorsCteBody` stack from `backend/src/hafsql.ts`) ∪ Route 3 (name-only `claim_authorship` + `approve_authorship` via the arm-less `authorshipClaimsCteBody` — the former ORCID and hive auto-accept arms are deleted), minus demotions (`author_resign` / `revoke_authorship`, latest-op-wins). Credit stays keyed to the on-chain post identity (`user_papers` carries `chain_author`/`chain_permlink`; the deduped `chain_papers` CTE collapses multi-recipient posts; a dual-route account contributes exactly one `user_papers` row). "Canonical SQL Query" below tracks the running code.
>
> **Retroactivity invariant (binding).** Consent gates set-*membership*, never the vote window. Once an author is consented, the next cycle credits them the paper's FULL vote/review history — including votes and reviews cast BEFORE their accept — because scores are recomputed from scratch each cycle (`prevScores` only weights voters; it is not a per-author accumulator). Demotion likewise acts by removing membership at the next recompute, not by windowing votes. Do NOT introduce a `vote.block_num > accept.block_num` predicate (under any alias) into the vote/review aggregation: the invariant is pinned structurally by `reputation-consented-credit-cycle-sql-shape.test.ts` (consent-column denylist over the aggregation region) and behaviorally by `reputation-consented-credit-cycle-behavioral.test.ts` (a pre-accept vote earns the full paper score through the composed scoring output).
...
| `$17` | `text` | `config.hiveBridgeAccount` — bridge-author pin (`validPevoPaperWhere`); the claims builder binds its own bridge param for the `approve`/`revoke_authorship` signer gates (see the builder-allocation note below) |
...
| `$21`+ | — | **Builder-allocated.** `authorshipClaimsCteBody(21, { claimers })` binds `APP_TAG` again, `config.hiveBridgeAccount`, the claimer array, and `config.hiveAdminAccount` (the `approve`/`revoke` signer gates) at offsets relative to its start index; the consent stack (`consentChainCteBody({ rootsFromCte: 'consent_seed' })` → `consentedAuthorsCteBody({ signers })`) continues from the builder's `nextIdx`. Absolute positions are builder-internal and shift with builder changes — `hafsql.test.ts`'s param-arithmetic suite pins the relative layout; do not cite absolute slots here. |
...
**Authorship claims (co-author credit — Route 3, name-only slots; composed from `authorshipClaimsCteBody`)**

| CTE | Computes |
|-----|----------|
| `claim_events` | All `claim_authorship` / `approve_authorship` / `revoke_authorship` `custom_json` (`custom_id` = the builder's APP_TAG param): action, claimer, paper_author, paper_permlink, author_index, approver (`required_posting_auths[0]`), block_num. |
| `accred_ranked`, `active_accreditations` | From `activeAccreditationsCteBody(19)`: authority-gated (`required_posting_auths ?\| $20`) accreditations, ranked per account, latest `accredit` wins — the attested-ORCID source for Route-2 anchor eligibility in the consent stack (and display supersession). |
| `claims_`-prefixed chain backbone | The builder embeds its own cumulative-chain walk seeded from `claims_base` (bounded by claim cardinality) so `author_index` resolves against the **cumulative-chain display union**, not the root post alone. |
| `accepted_claims` | `DISTINCT (claimer, paper_author, paper_permlink)` for `target_users`' claims not voided by a qualifying revoke (signer ∈ paper_author / bridge / admin / claimer, builder-bound params), AND explicitly approved (signer ∈ paper_author / bridge / admin). The single `accepted` path requires the claim's `author_index` to resolve to a **name-only** slot (no `hive`/`orcid` anchor) in the cumulative-chain union — anchored slots consent via Route 2, never via claims. There is **no auto-accept arm** (the former ORCID and hive arms are deleted). |

**Consented authors (co-author credit — Routes 1/2; composed from `consentSeedCteBody` semantics via an op-derived seed + `consentChainCteBody` + `consentedAuthorsCteBody`)**

| CTE | Computes |
|-----|----------|
| `consent_seed` | Distinct root papers cited by the batch's `author_accept` / `author_resign` ops — bounds the chain walk by batch consent activity, not corpus size. |
| `chain_tree` → `ranked_children` → `canonical_chain` → display-slot CTEs | The recursive `WITH RECURSIVE` cumulative-chain walk: cumulative-admission gate threaded down the recursion, earliest-created canonical-path selection (`NULLS LAST`), 50-hop cap, visited-array cycle guard, two-track first-occurrence display-slot ordering (the `author_index` domain), append-only claimed slots with Rule-6 first-appearance blocks. JS twin: `buildCumulativeAuthorsForChain` (`routes/papers.ts`). |
| `consented_authors` | Route 1 (root broadcaster) ∪ Route 2 (`author_accept` where signer matches the slot's `hive` or the signer's authority-attested ORCID matches the slot's `orcid`; accept block strictly > the slot's first-appearance block per Rule 6), minus `author_resign`/`revoke` demotions, latest-op-wins per `(block_num, id)`. Declared `AS MATERIALIZED` (display-surface fence; a no-op here where it is multi-referenced). |

**Papers arm**

| CTE | Computes |
|-----|----------|
| `user_papers` | Native papers by target users (`continues IS NULL`, with the Route-1 demotion guard: a root who resigned or was revoke-named is dropped) ∪ accepted-claim papers crediting the claimer (Route 3) ∪ consented-author papers crediting the consenter (Routes 1/2 via `consented_authors`), deduped so a dual-route account contributes exactly one row. Each row carries the credit-recipient `author` plus `chain_author`/`chain_permlink` (the on-chain post identity). |
...
| `paper_vote_signals` → `paper_latest_votes` → `paper_resolved_votes` | Native votes + revote `custom_json` by accredited voters (`block_num < $6`) on `chain_papers` posts; latest-per-(voter, post) wins; poster self-votes, `authors[].hive` co-author votes, and credited-set self-dealing (NOT EXISTS over `accepted_claims` ∪ `consented_authors`) excluded. |
| `paper_reviews` | Per `chain_papers` post, the averaged 4-dimension rating → quality multiplier, over valid non-self accredited-or-anon (`$2`/`$18`) reviews, excluding credited-set self-reviews (NOT EXISTS over `accepted_claims` ∪ `consented_authors`). |
...
5. **Self-vote exclusion.** Paper votes exclude the author and co-authors (via `jsonb_array_elements` check on `authors[].hive`) AND any credited co-author's self-dealing — both the vote and review aggregations carry a NOT EXISTS gate over the full credited set (`accepted_claims` ∪ `consented_authors`), so a Route-2/3 credited co-author's self-vote or self-review never counts. Review votes exclude the reviewer (`voter != author`). Citation upvotes exclude the citing author. The display surfaces mirror the same credited-set exclusion via the `excludeClaimedSelfWhere` + `excludeConsentedSelfWhere` helper pair.
...
The reputation algorithm today aggregates by Hive account (`tu.username`, `authors[].hive`, the post `author`). It does NOT aggregate by ORCID. ORCID values surface in two narrow places: (a) the consent stack's Route-2 anchor matching (`consentedAuthorsCteBody` compares the signer's authority-attested ORCID from `active_accreditations` to the slot's `authors[i].orcid` to decide accept *eligibility* — an anchor match gates who may consent, never credit on its own), and (b) display fields on paper-detail responses (see `agents/docs/hive-schemas.md` § 1.1 "ORCID supersession rule"). Neither is an aggregation over ORCID values, so the supersession rule does not currently change any reputation score.
...
The Route-2 ORCID anchor match in the consent stack is correctly framed as a one-way eligibility trigger and is NOT an aggregation. A matching attested ORCID only makes the signer eligible to consent to slot `i` via their own `author_accept`; the consented account is then credited via the existing hive-keyed aggregation (`consented_authors` arm of `user_papers`). The anchor comparison resolves ONLY through the authority-attested map (membership resolution over `active_accreditations`); a broadcaster-claimed-but-unattested ORCID never anchors, and a null attested ORCID against an orcid-anchored slot suppresses eligibility.
