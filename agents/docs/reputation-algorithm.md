...
Authors with accepted authorship claims on a paper receive the same paper reputation score as the posting author (shared co-author credit, not divided). The paper's votes, reviews, decay, and quality multiplier all apply identically. Implemented via `accepted_claims` in the reputation query, which resolves claim status with the same logic as `authorshipClaimsCteBody` (explicit approval, ORCID auto-accept, hive-username auto-accept, with revocation override). Credit is keyed to the **on-chain post identity**: `user_papers` carries `chain_author`/`chain_permlink` on both the native and claim arms, a deduped `chain_papers` CTE collapses a post credited to several recipients, and the vote/review CTEs join on the chain identity while the final SUM attributes the score to the claimer (`up.author`). Votes and reviews signed against the original post therefore credit the claimer, and a multi-recipient post is not multiplied. The ORCID auto-accept arm sources the claimer's attested ORCID from the authority-gated `active_accreditations` set (composed via `activeAccreditationsCteBody`, whose `accred_ranked` CTE trusts an `accredit`/`revoke` op only when its `required_posting_auths` includes an accreditation authority), the same gated source the `authorshipClaimsCteBody` read surface joins, so a self-signed accreditation cannot drive a co-author credit and the cycle and read surfaces agree on which ORCID attestations are valid. Self-claims (claimer = post author) are excluded to avoid double counting.

**Authorship list is final (design invariant, 2026-06-05).** Credit binds only to an author slot named at posting; there is no "unlisted claim → approve → add a never-named co-author" path (see `hive-schemas.md` § 2.9/2.10). New co-authors are added only via continuation revisions. Two enforcement changes are landing via the held `backend-co-author-claim-zero-score` task and are NOT yet in the query: (1) the explicit-approval arm must require `author_index` to resolve to a named slot (it currently accepts unlisted claims); (2) the paper self-vote and self-review exclusions must reject any `accepted_claims` claimer for the chain post, not only the chain poster and `authors[].hive` members — otherwise an ORCID-matched or name-only-slot claimer can self-vote/self-review the paper they are credited for.
