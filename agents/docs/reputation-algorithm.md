...
**Authorship list is final (design invariant, 2026-06-05).** Credit binds only to an author slot named at posting; there is no "unlisted claim → approve → add a never-named co-author" path (see `hive-schemas.md` § 2.9/2.10). New co-authors are added only via continuation revisions, which name them (claimed) before they consent (credited). Two enforcement changes remain **pending** in the live query (held `backend-co-author-claim-zero-score`, to be folded into the consented-set migration): (1) a name-only claim's `author_index` must resolve to a named slot (the query currently accepts unlisted claims); (2) the paper self-vote and self-review exclusions must reject any consented claimer for the chain post, not only the chain poster and `authors[].hive` members — otherwise a claimer can self-vote/self-review the paper they are credited for.
