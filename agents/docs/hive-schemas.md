...
- `continues` — present only on continuation posts (new permlink continuing a paper when the original author differs from the editor). Points to the current chain head being continued. Absent on initial publication and same-author edits. SQL queries filter out continuation posts (`(json_metadata -> APP_TAG -> 'continues') IS NULL`) to find canonical papers. Known design-alignment residual (P3, recorded): a present-but-malformed `continues` (wrong shape, dangling target) is NOT a root under the `IS NULL` gate, while upward seed walks that cannot follow the pointer (the `/api/me/authorships/pending` discovery seed) stop there and treat the post as a chain root — the divergence affects discovery seeding only, never credit, because the authoritative down-walk re-derives admission from real roots.
...
**Claim/approval binds accounts to named slots; it never adds a name.** The slot domain `author_index` resolves against is the **cumulative-chain author union** — the display-ordered union of `authors[]` entries across the paper's whole continuation chain (root post plus every admitted revision; see § 2.10 and `ARCHITECTURE.md` § 2 "Display construction"), NOT the root post's array alone. A name-only co-author first named in a continuation revision is therefore claimable. New names still enter only through a continuation revision that names them (see `ARCHITECTURE.md` § 2 "Authors mutation"), never through claim/approval. A claim whose `author_index` does not resolve to a slot in the cumulative union grants no authorship credit. Resolution against the root post only MUST NOT be re-introduced — the production resolver is the cumulative-chain walk in `authorshipClaimsCteBody`'s embedded chain backbone.
...
- `author_index` — zero-based index into the paper's **cumulative-chain author union** (two-track first-occurrence display ordering; see § 2.10) identifying the name-only slot the claimer is claiming. **Required**: the claim must reference a slot named somewhere in the chain that carries no `hive`/`orcid` anchor. A claim that does not resolve to a slot in the cumulative union grants no authorship credit. Slots carrying a `hive` or `orcid` anchor are consented via `author_accept`, not this route — there is no metadata auto-accept.
