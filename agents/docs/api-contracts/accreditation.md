...
Both short-circuit branches return the same envelope shape (`{ message, username, tx_id, outcome }`); only the `outcome` discriminator and the `tx_id` semantics differ.
