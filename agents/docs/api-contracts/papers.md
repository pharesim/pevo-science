...
- `authors[]` — on a multi-link continuation-chain paper this array is the **cumulative union** of `pevo.authors[]` across every post in the chain (root + continuations + native edits), in first-occurrence order, not a projection of the head post's metadata. An author credited on any chain post therefore stays in `authors[]` even if a later edit drops them from one post. Single-link papers project their own post's `pevo.authors[]` directly. See `ARCHITECTURE.md § 2 "Display construction (cumulative union)"` for the full construction (two never-merging tracks, sub-field resolution, ORCID/name supersession). The list and detail surfaces resolve to the same union via the shared `resolveChainCumulativeAuthors` helper.
- `authors[].name` — **mandatory** (always a non-empty string). Resolved at read time by name-supersession + a fallback chain: an accredited author's accreditation-attested `researcher_name` supersedes the broadcaster-claimed name **silently** (no discrepancy field and no audit event, since name variation is benign), otherwise the order is broadcaster `name`, then hive handle, then `orcid`. An entry resolving none of these is dropped (it names no one). Unlike ORCID, name-supersession adds no new field and surfaces no "claimed vs verified" indicator. See [hive-schemas.md § 1.1](../hive-schemas.md).
- `accredited_authors` — the intersection of the **Hive-keyed** author union with currently-accredited accounts (latest `accredit`/`revoke` wins). Rebuilt from the same cumulative union as `authors[]`, so it reflects every chain post's accredited co-authors, not just the head's. Hive-less display credits (`hive` absent/unnormalizable) have no account to be accredited and never appear here.
...
- `authors[].orcid_verified` — the accreditation-attested ORCID for the author's hive account, looked up at read time against `active_accreditations`. Present and non-null when `authors[].hive` is set, that account is currently accredited (per `active_accreditations`, the sanction-aware live-membership view; see `accreditation-state-read-latest-action-wins-2026-05-15.md`), and the accreditation record carries a non-empty `orcid`. Null when the hive is empty, the account is not currently accredited, or the accreditation carries no ORCID. The canonical display rule is "use `orcid_verified` if non-null, else fall back to `orcid`". **Cache staleness:** `orcid_verified` and `orcid_discrepancy` MAY be up to 30 minutes stale relative to current `active_accreditations` state (the paper-detail cache TTL); accreditation grant / revoke / ORCID-rotate events propagate on the next cache miss. This is the chain-is-SSoT / cache-is-perf-layer posture. The consented-set badge (`ARCHITECTURE.md` § 2 "Consented vs claimed authorship", surfaced as `authors[].consented` below) has tighter freshness guarantees and is an independent signal: supersession-fields are account-keyed attestations, the consented-set is paper-keyed authorship consent.
- `authors[].orcid_discrepancy` — `true` when both `orcid` and `orcid_verified` are non-null and they differ. Clients SHOULD render a discrepancy indicator (e.g., a tooltip showing both values labeled "claimed" and "verified"). `false` otherwise. **Continuation-chain caveat:** On continuation-chain papers where the server-override mechanism fires (per `ARCHITECTURE.md` § 2 "Display construction (cumulative union)"), `orcid` in the response is the post-override value (the attestation), while `orcid_discrepancy` reflects the PRE-override comparison sampled before the override mutates the response payload. Therefore on these papers `orcid` MAY equal `orcid_verified` while `orcid_discrepancy` is `true` — this combination indicates the server corrected a broadcaster-claimed mismatch in-band. Clients treat the discrepancy signal as the authoritative audit indicator regardless of whether `orcid` and `orcid_verified` appear equal in the response.
- `authors[].consented`: boolean present on every paper-detail author entry (absent from list / summary responses). `true` when the author is consented via Routes 1/2 of the consented-authorship model (`ARCHITECTURE.md` § 2 "Consented vs claimed authorship"): the root broadcaster (implicit consent), or an anchored slot whose owner broadcast `author_accept`. The flag is hive-keyed, so a hive-less entry (a name-only display credit or an unresolved-ORCID slot) is always `false` here, even when its owner is credited by the reputation cycle through a Route-3 accepted claim. Render the author credit badge from the union of `consented` and an `accepted` `authorship_claims[]` entry for the same slot, never from `consented` alone. Bridge papers report `true` only for the bridge account; a single-author root paper reports `true` for its sole root entry. Freshness: at-most-one-block stale (computed outside the 30-minute stable detail cache). Fail-closed: a HAF outage makes the whole detail response a retriable 503 rather than emit `consented: false` for legitimate co-authors.
- `review_count` — number of reviews on this paper satisfying the canonical PEvO review-validity gate (`type='review'`, accredited author or anon-proxy, structurally-valid 4-dim rating object with each dimension an integer in `[1,5]`, not authored by the paper author, a named co-author, or a credited authorship-claimer holding an `accepted` claim on this paper) AND whose parent post is a valid PEvO paper (native pevo paper or `bridge_paper` authored by `config.hiveBridgeAccount`). Authoring client (`app` field) is not gated. Source of truth: `validReviewWhere()` AND `validPevoPaperWhere(source:'all')`, composed with `excludeSelfReviewWhere()` (paper author + named co-author) and `excludeClaimedSelfWhere()` (accepted-claim claimer), in `backend/src/hafsql.ts` (all gates compose at every review-class surface for display↔reputation parity).
...
- `authors[]` / `authors[].name` / `accredited_authors` — same cumulative-union, mandatory-`name` (name-supersession + fallback chain), and Hive-keyed-intersection semantics as `PaperSummary` above. On multi-link papers `authors[]` is the cumulative union across the chain (first-occurrence order); `name` is always present (attested → broadcaster → hive → orcid). PaperDetail additionally carries `authors[].affiliation`.
...
- `authorship_claims[]` — list of non-revoked Route-3 (name-only slot) authorship claims on this paper. `status` is `accepted` (the post author, or the admin / an approved co-author on bridge papers, broadcast `approve_authorship` against the claim's name-only slot) or `pending` (claimed via `claim_authorship`, not yet approved). There is no ORCID or hive auto-accept: acceptance always requires an explicit `approve_authorship` op resolving to a name-only slot (`ARCHITECTURE.md` § 2; `hive-schemas.md` § 2.9 to 2.11). Revoked claims are excluded. An `accepted` entry credits its `claimer` even when that claimer is absent from `authors[].hive`; combine it with `authors[].consented` for the full per-author credit signal.
