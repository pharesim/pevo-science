...
- `new_review` intentionally fires for a credited authorship claimer (matched by ORCID, or connected to a name-only slot, and so absent from `authors[].hive`) who reviews the paper they are credited for, even though that review is excluded from the display review aggregates (`avg_rating`, `review_count`, the review list) and from the reputation score. A notification confers no rating weight and no credit, so the arm reports the raw review activity; the display surfaces apply an additional accepted-claimer exclusion (`excludeClaimedSelfWhere`) that this arm deliberately does not. A `new_review` can therefore refer to a review that does not appear in the paper's review aggregates.
