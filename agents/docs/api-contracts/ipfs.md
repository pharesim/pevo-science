...
```json
{
  ...
  "size": 2048576,
  "fresh_auth_proof": "<session fresh-auth proof; JWT path only>"
}
```
...
- **JWT (light-account) path:** a single-use session `fresh_auth_proof` is required in addition to the JWT, per ARCHITECTURE.md § 6.5 invariant #1 (a replayable JWT alone must not reach a critical action). Mint one via `POST /api/custody/fresh-auth` (password) or the ORCID fresh-auth round-trip.
...
**Errors:**
- `FRESH_AUTH_REQUIRED` (401, or 403 on a username mismatch) — JWT path with a missing or invalid `fresh_auth_proof`.
