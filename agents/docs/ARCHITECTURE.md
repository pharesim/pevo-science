...
**Implementation status.** The continuation-admission gate (claimed-membership) and the cumulative-union display construction are **live**. The **vouched** layer (`author_accept` / `author_resign` consent ops gating continuation admission, reputation flow, citation credit, and continuation-edit power) is **Phase 2**: its read-time computation primitives (`computeVouchedAuthors` / `fetchConsentOpsForPaper` in `backend/src/consent-ops.ts`) are implemented but not yet wired into any gate. Statements below that describe vouched-gating as operative describe the Phase 2 target, not current behavior; the live gate admits on **claimed** membership (see "Display construction" and "Vouched-set computation (Phase 2 constraints)").
...
The continuation-author-consent gate currently admits continuation posts from any **claimed** author of the paper (membership in the cumulative chain `pevo.authors[].hive` set, per "Display construction" below); this is the live defense against the outside-attacker spoof. Additionally gating continuation admission, reputation flow, citation credit, and continuation-edit power on **vouched** status (rather than merely claimed status) is the Phase 2 tightening described under "Implementation status" above. Once the vouched layer is wired in, a claimed-but-unvouched name shown on the paper will be informational only.
...
The two tracks **never auto-merge**. A Hive-less display credit is never linked to a Hive identity by fuzzy name or ORCID matching. The only path from Hive-less credit to a vouched Hive identity is the explicit bridge-author-claim attestation flow (`backend-bridge-paper-author-claim-flow`); importer-side or read-time auto-mapping is forbidden (a dormant `author_accept` pre-broadcast under a colliding handle would otherwise activate retroactively — see "Bridge papers" below).
...
| `title`, `body`, `abstract`, `citations`, `keywords`, `discipline`, `tags`, `language`, `supplementary_files`, `addresses_reviews` | Free-edit by any admitted continuation author (claimed-membership gate today; vouched-gated in Phase 2). Risk accepted. Deterrents: on-chain audit (broadcaster-attributed), accreditation revocation, original-author re-edit power. |
...
Hard cutover. From the flag-day deploy of the vouched layer (Phase 2), vouched status requires either root-broadcast or an on-chain `author_accept` op. Existing multi-author papers' co-authors must broadcast `author_accept` to retain vouched status; until they do, they are demoted to claimed-pending. Under the current claimed-membership continuation gate they can still broadcast admitted continuations; once the vouched layer is wired into `resolveContinuationChain`, claimed-pending authors can no longer do so. Single-author papers are unaffected (the broadcaster is implicitly vouched).
...
| Issue IPFS upload token (binds a file to the auth envelope before pinning) | `POST /api/ipfs/upload-token` | Signature path: the per-request Hive signature body-hashes the declared file descriptor (`{file_sha256, mimetype, size}`) into the signed envelope. JWT path: a single-use **session** fresh-auth proof (`fresh_auth_proof` in body), so a stolen JWT alone cannot mint a token. The subsequent `POST /api/ipfs/upload` carries the returned token in `X-Upload-Token` and is rejected unless `sha256(file)` matches the declared hash. | All accredited accounts (light A/B/C/D and self-custody/Keychain). Implemented at `ipfs.ts` (`/upload-token`). The session-proof choice (vs a per-action target binding) is under review in `backend-ipfs-upload-token-proof-binding`. |
