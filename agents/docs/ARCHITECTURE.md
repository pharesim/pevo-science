...
**Implementation status.** The continuation-admission gate (claimed-membership) and the cumulative-union display construction are **live**. The **consented** layer — explicit per-paper consent (via the two routes in "Consented vs claimed authorship" below) gating reputation/citation credit and the display badge — is the model documented here but **not yet wired**: the live reputation cycle still credits co-authors via the legacy `claim_authorship` / `approve_authorship` resolution with metadata auto-accept (see `reputation-algorithm.md` "Co-author Credit"), and the read-time consent primitives (`computeVouchedAuthors` / `fetchConsentOpsForPaper` in `backend/src/consent-ops.ts`, to be renamed when wired) are implemented but consumed by no gate. Statements below that describe consent-gated credit as operative describe the target model, not current behavior; the live credit path is the legacy resolution and the live continuation gate admits on **claimed** membership (see "Display construction" and "Consented-set computation (Phase 2 constraints)"). Backend implementation is tracked as a separate task — docs are intentionally ahead of code.
...
Once the consented layer is wired, the HAF read fails closed. When `getPool()` returns null or the consent-op query throws, paper-detail and `/api/me/authorships/pending` MUST return 503 INTERNAL_ERROR rather than degrade to a root-only consented-set. Degrading would silently demote legitimate co-authors below the cumulative-union claimed-set baseline AND open an attacker-attractive bypass window during HAF flaps; "chain is SSoT" is binding here. The integration site MUST short-circuit the consent fetch when the consent flow is inert (single-author claimed-set or bridge papers per the "Bridge papers" subsection) so the fail-closed surface is bounded to genuinely multi-author papers. The `fetchConsentOpsForPaper` helper MUST distinguish "no ops" from "HAF unavailable" via its return type so the integration site applies the policy explicitly. Operators see the outage as HTTP 503s plus a per-request structured pino log marking the fail-closed event. This posture matches the existing HAF-required reads at `verifyOrcidBinding` (`backend/src/routes/orcid.ts:1502-1506`, "Fail closed when HAF is unavailable: returning null would silently bypass...") and the chain-walk SQL in `resolveContinuationChain`, which is HAF-required by construction.
...
| Server-side broadcast (name-only-route credit ops: `claim_authorship`, `approve_authorship`, `revoke_authorship`) | `POST /api/custody/broadcast` | Per-target fresh-auth proof (target binds `op_type` + `paper_author` + `paper_permlink` + `author_index`) | Same as non-consent. **Pending** — these reputation-weighty credit ops are not yet fresh-auth-gated; tracked by `backend-authorship-credit-ops-fresh-auth`. |
