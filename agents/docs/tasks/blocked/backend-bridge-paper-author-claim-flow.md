...
**Near-term defensive guard filed separately:** `backend-bridge-exclude-route2-orcid-consent` (since landed and archived; the bridge exclusion is live in `hafsql.ts`'s consent-signer-eligibility CTE, with a real-postgres corpus test) adds the bridge exclusion to the live `consentedAuthorsCteBody` Route-2 ORCID arm + a corpus case where a bridge slot ORCID matches an attested account (asserting it confers NO consent). No live defect today (the real-postgres corpus pins only the no-match case), but the boundary becomes reachable as bridge-with-ORCID imports and ORCID attestations both grow, so it is closed now rather than left to coincide. This is the "if the latter, the eligibility arm needs a bridge exclusion plus a corpus case" action from the 2026-06-11 note.
