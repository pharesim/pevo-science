...
parent task's hold block. This task tests
`findCustodyBroadcastByIdempotencyKey` and
`findAccreditationBroadcastByIdempotencyKey` against real HAF — both
symbols are in the parent's round-2 commit `689208f` but not yet
archived. If the architect's re-review surfaces a rename or signature
change on those helpers (the F23 rename was already part of the
parent; subsequent re-review might find more), this task's spec
would need to follow.

The parent task's F6 part-1 carve-out header in
`tests/lib/idempotency.test.ts` already cites THIS task slug as the
real-path coverage commitment. Coupling is explicit: the parent's
test-header citation locks in the eventual integration-test scope.
