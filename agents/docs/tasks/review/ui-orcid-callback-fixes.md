...
## UI re-review signal (2026-04-28, commit `1c28b39`)

Round-2 hold items #1-4 landed. Ready for architect re-review.

- **Item #1 (P2 — `_handleLogin` polling parity):** `frontend/src/pages/orcid-callback.js` `_handleLogin` now calls `auth._startAccreditationPolling()` instead of bare `auth._checkAccreditation()`. Matches sibling `loginFromResponse` / `connect` paths in `auth.js` so a transient accreditation-fetch failure no longer pins the store at `isAccredited=false` permanently. Comment in code explains the parity rationale. Investigation: the line 341 `_checkAccreditation()` site is inside `_handleAccredit` (the accreditation-success path, not a login path) — left untouched per spec.
- **Item #2 (P3 — localStorage payload assertion):** `frontend/tests/unit/pages-orcid-callback.test.js` stale-state regression test extended with `expect(JSON.parse(localStorage.getItem('pevo_session'))).toMatchObject({ isAccredited: false, accreditation: null })`. The `_saveSession` mock is extended via `mockImplementationOnce` to ALSO write the full session payload (simulating what real `_saveSession` does), so the end-to-end persistence assertion is meaningful given the auth-store-mocked harness. Inline comment notes this on the mock.
- **Item #3 (P3 — call-count assertions):** Both the "handles login mode" test and the stale-state regression test now include `expect(mockAuthStore._saveSession).toHaveBeenCalledTimes(1)`. The login-mode test additionally asserts `expect(mockAuthStore._startAccreditationPolling).toHaveBeenCalledTimes(1)` (replacing the prior bare `_checkAccreditation` assertion).
- **Item #4 (P3 — dead fake timers):** Removed `vi.useFakeTimers()` / `vi.useRealTimers()` from the stale-state regression test — the test never advanced timers, never asserted navigation, never depended on setTimeout.
- **Test-harness updates:** `mockAuthStore` extended with `_startAccreditationPolling: vi.fn()`. The "_handleLogin direct-call post-teardown" test now also asserts `_startAccreditationPolling not called` (alongside the existing `_checkAccreditation not called` belt-and-suspenders assertion) to guard the self-mounted-guard contract.
- **Test results:** `pages-orcid-callback.test.js` 41/41 pass. Full frontend unit suite 987 pass (one pre-existing `sec-001-equivalence.test.js` transform failure unrelated to this change — fails identically on a stash-clean checkout). `npm run build` clean.
