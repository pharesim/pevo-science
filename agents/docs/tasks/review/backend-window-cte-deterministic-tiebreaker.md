...
All four items landed; moving back to `tasks/review/`.

- **Item 1 (missed site — `getAccreditedSet`).** Took the architect-sanctioned append: the inlined `ranked` ROW_NUMBER in `accreditation.ts` now orders `block_num DESC, cj.id DESC` with the convention-path anchor comment. This matches `activeAccreditationsCteBody`'s partition key and order exactly, so `getAccreditedSet` and the `active_accreditations` CTE now resolve every account identically on a same-block accredit/revoke (the disagreement the item flagged). Chose append over the reuse-the-CTE refactor to avoid changing the hot-path query's whitelist-filtered execution plan; the correctness/agreement concern is fully resolved by the matching tie-breaker.
- **Item 3 (missed site — accreditations route).** Appended `, cj.id DESC` + the convention comment to the list-endpoint ROW_NUMBER (`fetchAccreditationsFromHaf`). Also fixed the sibling per-username status query (`fetchAccreditationStatusFromHaf`, `ORDER BY cj.block_num DESC LIMIT 1`) in the same file — same latest-wins-per-account class, same display surface, already projected `cj.id AS event_id`; left unfixed it would be the exact partial-fix drift this task exists to prevent.
- **Item 2 (canary extension).** The SQL-shape canary in `window-cte-deterministic-tiebreaker.test.ts` now also asserts `activeVouchesCteBody().sql` carries `ORDER BY cj.block_num DESC, cj.id DESC` (imported the fragment). Added an in-test note that the three reputation union CTEs (`paper_latest_votes` / `review_latest_votes` / `citing_latest_votes`) are inlined, not exported as fragments, so the shape canary cannot reach them; they are covered end-to-end by the real-HAF reputation-lifecycle suite.
- **Item 4 (comment reword).** The three reputation union-CTE tie-breaker comments now document the GLOBAL `haf_operations`-PK invariant (one monotonic sequence shared across `operation_vote_view` and `operation_custom_json_view`), state that `op_id DESC` across arms is genuine cross-arm latest-wins, and warn not to namespace `op_id` per arm.
- Verification: `npm run typecheck` + `npm run lint` clean. `window-cte-deterministic-tiebreaker.test.ts` 3/3 green against real Postgres (the two `skipIf(!isHafConfigured())` behavioral cases executed, not skipped). Accreditation regression set (`accreditation`, `accreditations`, `accreditations-revoke`, `active-vouches-signer-gate`, `accreditation-orcid-cache`) green except the pre-existing load-induced flake in `accreditation.test.ts`'s concurrent broadcast-attempts-cap case — confirmed pre-existing by stashing all backend src edits and reproducing the same failure (different non-deterministic value each run: 0/1/6) on the pristine tree; that path (`routes/accreditation.ts` + `lib/idempotency.ts`) is untouched by this task.
