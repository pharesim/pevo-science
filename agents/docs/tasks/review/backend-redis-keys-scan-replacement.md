...
## Backend re-review signal (2026-06-09) — round-3 hold items 1-3 landed

1. **`as const` on the test seams.** Added `} as const;` to the `reputation.ts` `__test_seams` export AND the pre-existing `reputation-batch.ts` `__test_seams` export.
2. **Unconditional SADD in `seedAccreditationBonus`.** Dropped the `if (setResult === 'OK')` gate; the SADD now runs unconditionally after the NX SET. SADD is idempotent, so re-indexing an already-present member costs one round-trip and closes the orphan window (a crash between a prior SET and its separate SADD leaves the key unindexed; a retry's NX no-op would otherwise skip the SADD on a false "already a member" assumption). The surrounding comment is rewritten to state this rationale, anchored on behavior.
3. **Write-path pins.** Added a `members-index write paths` describe to `reputation-batch-internals.test.ts` (reusing the `setBatchMembersKey(TEST_MEMBERS_KEY)` isolation): (a) a fresh `seedAccreditationBonus` SADDs the prod key (`sismember === 1`); (b) the orphan-window case — prod key present but unindexed, NX SET no-ops — still SADDs (`sismember === 1`), which mutation-kills the previously-gated form; (c) `invalidateOnRevocation` SREMs the member (`sismember === 0`) after the DEL. Comments anchored on behavior (no round/hold ordinals in test source).

Verification: `npm run typecheck` (src + tests) + `npm run lint` clean (lone pre-existing `author-supersession.ts` unused-directive warning untouched). `reputation-batch-internals.test.ts`: the 3 new write-path tests pass. The one failing test in that file (`runBatchComputation invokes batchMapToScoreRecord on the default-param path`) is environmental — it requires a real HAF head block beyond genesis + cycle_blocks, which the local HAF→pevo_app redirect cannot provide; it fails identically on clean HEAD with these changes stashed, so it is not a regression (it was green in the real-HAF re-review environment that listed this file green).
