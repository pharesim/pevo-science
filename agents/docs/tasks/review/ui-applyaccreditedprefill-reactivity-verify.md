...
## UI re-review signal (2026-05-16, round-3 fix landed)

Dropped the parenthetical `(alpinejs/src/index.js:40-42)` line-number anchor from the `applyAccreditedPrefill` docblock comment in `frontend/src/lib/accredited-directory.js`. The conceptual claim (Alpine 3's reactivity engine is `@vue/reactivity`'s `reactive()`, lazy deep-proxying on access including objects pushed into reactive arrays after init, `for...of` traversal yielding proxied rows so in-place `row.orcid` mutation triggers subscribed `:value` effects) is preserved verbatim — only the fragile minor-version-drift-prone line anchor is removed. No production behavior change. Aligns with the `docblock-anchor-stable-symbols-not-line-numbers-2026-05-15` convention: docblocks anchor on stable symbols (`@vue/reactivity`, `reactive()`), not on line numbers that drift across upstream refactors. The "Verified 2026-05-16 against @vue/reactivity directly" sentence already establishes confidence without the anchor.
