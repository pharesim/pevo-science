...
Both round-4 hold items landed (coordinated with `backend-digest-share-line-terminators` in ONE commit, per the hold's instruction):
1. `digest.ts` `singleLine` docblock `\s`-membership corrected. Verified empirically on Node 20: only NEL (U+0085) and now FS/GS/RS (U+001C-U+001E) are non-`\s` members; VT/FF/LS/PS ARE `\s` members the second `\s+` collapse would catch. The docblock now justifies the explicit first pass by NEL + FS/GS/RS only.
2. `papers-cite-escape.test.ts` extended-alphabet block mirrors the C0-separator widening: FS/GS/RS added to `SEP`, `ANY_SEP`, and the `SEPARATORS` table (each flattens in `bibtexEscape`/`risEscape`/`singleLine`), plus a new RIS test (FS/GS/RS cannot emit a second TY/ER/AU record) and a new BibTeX test (a GS-smuggled header cannot create a second `@article{`). Separators built via `String.fromCharCode`.

`npm run typecheck` + `npm run lint` clean (lone pre-existing `author-supersession.ts` unused-directive warning, not in scope); `papers-cite-escape.test.ts` + `digest-title-strip.test.ts` green (79/79).

## Architect re-review (2026-06-09) — HELD PENDING FIXES:

Round-5 review of commit `1577347b` (the coordinated round-4 hold-fix) by a 7-persona `/ce-code-review` (correctness/security on Opus; testing/maintainability/project-standards/kieran-typescript/learnings on Sonnet). **The round-4 items are confirmed FIXED:** the `\s`-membership prose is now factually correct in both docblocks and both test labels (verified empirically on Node 20 — only NEL + FS/GS/RS are non-`\s`; VT/FF/LS/PS ARE `\s` members), the regex widened to exactly the 10 intended code points (correctness BMP-scanned; security cross-checked Python `str.splitlines()`, US/NUL correctly excluded), the RIS/BibTeX forgery is closed, and the `papers-cite-escape.test.ts` C0-separator mirror landed. The sibling `backend-digest-share-line-terminators` is ARCHIVED. Two residual cleanup items before this task archives — both land in THIS task's files; coordinate them into ONE backend commit:

1. **The new RIS C0-forgery test is false-confidence (correctness + testing, conf 100).** The `it('RIS: a C0-separator-smuggled record (FS/GS/RS) cannot emit extra TY/ER/AU lines', …)` test in `papers-cite-escape.test.ts` splits `generateRis` output on `\n` only and asserts `TY`/`ER`/`AU` line cardinality. Because `generateRis` joins records with `\n` (not with FS/GS/RS), a surviving FS/GS/RS stays mid-line inside the `TI  - ` value and the 1/1/1 counts hold even if the FS/GS/RS widening is reverted — so the test passes against the un-widened regex and proves nothing about the C0 closure it is named for. Add an `expect(generateRis(detail)).not.toMatch(ANY_SEP)` assertion (the file-local non-`/g` separator regex, which now includes FS/GS/RS) so the test goes RED on revert, mirroring the sibling `it('BibTeX: a C0-separator-smuggled entry header (GS) cannot create a second entry', …)` test two blocks up. Keep the existing `TY`/`ER`/`AU` cardinality checks as defense-in-depth. (Note: the FS/GS/RS closure itself is NOT unguarded — the parametrized `risEscape` per-separator table rows and the BibTeX/digest C0 tests already go red on revert; this is a named-test-must-prove-its-own-claim gap, not a coverage hole.)

2. **Stale consumer docblock in `routes/papers.ts` undercuts the single-source-of-truth story.** The block comment introducing the shared constant still reads "`LINE_TERMINATORS` is the shared 7-character separator alphabet (CR, LF, VT, FF, NEL, LS, PS)" — the constant is now the 10-char class. The round-4 widening swept the `digest.ts` consumer docblock but missed this `papers.ts` one (the `line-terminator-flattening-alphabet-and-regex-s-membership-2026-06-08` learning names BOTH consumer docblocks as mandatory audit targets when the constant is edited). This stale comment is in committed HEAD. Update the enumeration to the 10-char class (add FS, GS, RS) so the consumer comment matches the widened constant. **Staging caution:** `routes/papers.ts` currently carries UNRELATED sibling working-tree edits far from this comment — stage only the one-line comment change (`git add -p`), do not sweep the sibling's in-flight hunks.
