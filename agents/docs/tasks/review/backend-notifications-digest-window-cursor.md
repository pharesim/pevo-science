...
`/ce-code-review` (8-persona fan-out on commit `79d91fe8`; Opus correctness + adversarial; testing/reliability/maintainability/project-standards on Sonnet). **The core fix is verified CORRECT:** removing the `!batch.has_more` gate and advancing to the highest delivered block on every non-empty run correctly kills the P1 re-send cascade; correctness confirmed the empty-array access is guarded, the ascending order makes the last element the true max block, the whole-block drop fires on the digest's `asc` path, `filterEventsAfter` is strict `>`, and the empty-batch case holds the cursor without a partial advance; adversarial confirmed the original boundary-overflow drop is NOT re-introduced and the cursor advances monotonically (sustained-window lag is late delivery, not loss). But this is a re-review, and **the 2026-06-08 hold's items (a)-(d) did not land** (the landing note confirms the change was "confined to digest.ts + test"), plus three accuracy items surfaced.

1. **(P2, comment + test accuracy — 4 reviewers) False recovery invariant for the single-block-over-cap case.** The cursor-advance comment in `runDigest` and the empty-batch "deferral" test both claim a dropped over-cap block "surfaces once the window floor slides to contain it." That is true for a MULTI-block truncation (the dropped newest whole block resurfaces on a later run because the cursor has not advanced past it), but FALSE for a true single-block-exceeds-cap burst: the forward-only floor ages that lone block OUT of the window, and its event count is fixed, so it is a PERMANENT drop, never delivered. Correct the comment to distinguish the two cases, and rework the empty-batch test so its name and assertions do not certify a permanent drop as a graceful deferral (e.g. drive a multi-run scenario advancing the simulated head and assert the lone block is NOT eventually delivered). The same false claim also lives in `fetchNotificationsFromHaf`'s docblock (the "for 'asc' the dropped newest block resurfaces" and "the block surfaces once the floor slides" lines) — correct it there too while in the area, anchored on behavior. Note: a >cap single block to one recipient in one 3s block is near-unconstructible, so this is a documentation/test-accuracy fix, not a live data-loss fix — but the false invariant will mislead the next person reasoning about cursor advancement.
2. **The 2026-06-08 hold's minor items (a)-(d) still open** (grep-confirmed):
   - (a) Document the defect-1 residual: edits of content published more than the window depth (~100k blocks) ago still re-fire once, because the publication row falls below the wide window floor so the per-arm DISTINCT ON has nothing to collapse the edit against. Document this near the `windowFloor` / `filterEventsAfter` logic in `runDigest`.
   - (b) Cold-start `getLastBlock() === 0`: add the early-return guard the prior hold requested, OR explicitly document-and-accept. Address the nuance: with a real genesis (`getGenesisBlock() > 0`) the floor falls back to `genesis - 1` (bounded), so the genesis-wide "all users silently skipped" scan requires `getGenesisBlock()` to ALSO return 0 (degraded-DB cold start). State the chosen resolution in the code.
   - (c) Drop the `export` on `NOTIFICATION_WINDOW_BLOCKS` in `notification-queries.ts` (zero external consumers — it is only used internally by `computeNotificationWindowFloor`) and trim its comment, which overstates the sharing scope.
   - (d) Fix the clause-(c) companion references in the `digest-window-cursor.test.ts` header to include the `routes/` subdir (`notifications-arm-sql-shape.test.ts` -> `routes/notifications-arm-sql-shape.test.ts`, and the sibling `routes/` references).
3. **(P2, maintainability) Stale comment anchor in `runDigest`.** The cursor-setup comment still says the digest "advances its cursor forward as whole blocks drain (see the advance-only-when-drained logic below)" — this commit removed that gate. Reword to the current behavior (advances to the highest delivered block every non-empty run).
4. **(P2/P3, testing) No test exercises `fetchNotificationsFromHaf` with `direction='asc'`.** The asc-path whole-block drop is the load-bearing contract the digest's unconditional advance rests on, and it is exercised nowhere (the digest test mocks `fetchNotificationsFromHaf` and assumes the contract; all real-path notification tests use the `desc` route path). Add a direct test (mocked-pool or real) asserting that on the asc/capHit path the NEWEST block is dropped whole, so a future asc/desc-asymmetry regression is caught.
5. **(P2, reliability — document, do not necessarily fix) Send/advance non-atomicity.** A successful `sendDigestEmail` followed by a failing `updateLastDigestBlock` re-emails the same slice next run; removing the gate makes this reachable on every non-empty run, not just on terminal drain. The duplicate-over-silent-skip tradeoff is defensible — add a one-line comment documenting it as the intended failure direction (no code change required unless you want an idempotency guard).
...
`/ce-code-review` fan-out on commit `eb44eba1` (correctness on Opus; testing, maintainability, project-standards, kieran-typescript on Sonnet; adversarial skipped — the only executable delta is the `NOTIFICATION_WINDOW_BLOCKS` export removal, the behavioral advance-logic was confirmed correct in the prior round; learnings folded from the shared route review; ce-agent-native skipped per PEvO). Reviewed as a pair with the route task `backend-notifications-route-newest-first-whole-block` since both touch `notification-queries.ts`. Tests verified green by the architect (24/24 across `notifications-window-cursor` + `digest-window-cursor` + `fetch-notifications-asc-whole-block`); typecheck + lint clean.

**All five hold items are VERIFIED landed and correct:** (1) the false-recovery invariant rework — the empty-batch test is now a multi-run scenario that advances the simulated head and asserts the lone over-cap block is NEVER delivered (testing confirmed it satisfies the prior hold's "do not certify a permanent drop as a graceful deferral"); (2) minor (a)-(d) all landed — defect-1 residual doc, cold-start head=0 document-and-accept, `NOTIFICATION_WINDOW_BLOCKS` export dropped (grep-confirmed zero external consumers), clause-(c) companion refs corrected to the `routes/` subdir (paths verified present on disk); (3) the stale cursor-setup anchor reworded to the current advance-on-every-non-empty-run behavior; (4) the new `fetch-notifications-asc-whole-block.test.ts` canary pins the asc/capHit whole-block drop — its three cases mutation-kill the asc/desc boundary swap (`events[events.length-1]` vs `events[0]`) and the `>= cap` vs `> cap` probe regression (this canary also retroactively closes the asc-firing-gate coverage the route reviewers flagged as missing in the route test file — it lives here); (5) send/advance non-atomicity documented as the intended duplicate-over-silent-skip direction. The export-drop, carve-out headers (clauses a/b/c), and comment-anchor cleanliness were all probed and confirmed.

One doc-accuracy item before archive:

1. **(P3 doc-accuracy; correctness + maintainability corroborated) Multi-block 'asc' recovery prose overstates recovery timing.** The rewritten Residual docblock in `notification-queries.ts` (the "For 'asc' (digest) ... the caller's cursor stops below it, so the next call re-fetches and delivers it whole (recovered)" line) and the `runDigest` cursor-advance comment in `digest.ts` (the "the dropped block resurfaces on the next run" line) attribute multi-block recovery to "the next call/run" because "the cursor stops below it." But `runDigest` fetches against the wide `windowFloor` (NOT the user cursor) and applies the cursor in-app via `filterEventsAfter`: on the immediately-following run the same oldest-`cap` batch is re-fetched, the partial newest block is dropped again, and `filterEventsAfter` strips the already-delivered older blocks, so the dropped block is NOT delivered next-call. Recovery is DEFERRED — the dropped block is delivered whole only on a LATER run, once the forward-sliding wide floor has aged the older delivered blocks below the floor so the previously-dropped block becomes the oldest-kept (no longer the cap-truncated end). The eventual-recovery invariant holds (no data loss); this is prose accuracy, not a runtime defect — but it is the same false-recovery-invariant class hold item 1 set out to make precise, so close it here rather than leaving a residual imprecision in the very prose this task corrected. Fix: reword BOTH locations so recovery is described as deferred-until-the-wide-floor-slides-past-the-older-delivered-blocks, and drop the "cursor stops below it → next call recovers" causal attribution (the wide floor, not the cursor, drives the fetch). Anchor the comments on the behavioral mechanism only — no task slug, round/hold number, line number, SHA, or `§` marker.

   Implementer note: a sibling reviewer's framing ("on a quiet account the truncation recurs indefinitely") is itself imprecise — the wide floor slides on chain time regardless of account activity, so recovery always eventually occurs. Describe the floor-slide as the recovery mechanism, not "newer events displace the dropped block."
