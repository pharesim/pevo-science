...
`/ce-code-review` (10-persona fan-out; correctness/security/adversarial at Opus tier) on commit `3fd7e938` confirmed all four prior hold items landed correctly: the in-process single-use lock (`inFlightConsumes`, synchronous `has`→`add` before the first await, released in `finally`) and the compensating `redis.del` on the memStore-fallback leg both mirror `fresh-auth.ts` and were verified sound under concurrent + Redis-flap interleavings (loser returns null without consuming; no double-spend; no dropped legitimate consume). The `X-Upload-Token` `string[]` handling, the AC4 `pending_ipfs_uploads` INSERT-args assertion, and the real-signature `/upload-token` body-tamper test all check out. One item before archive:

1. **Anchor-rot: memory-slug citation in a production comment.** The new `inFlightConsumes` docblock in `backend/src/lib/ipfs-upload-token.ts` cites a memory slug as a pointer. Per root `CLAUDE.md` "Comment anchors", code comments should anchor on the stable invariant itself, not an external slug. State the invariant inline instead (e.g. "this deployment is single-process; a multi-instance topology would re-open the race and require a Redis-side sentinel"), dropping the slug. `backend/src/lib/fresh-auth.ts` carries the same memory-slug citation pre-existing; fixing it there in the same pass would keep the two consistent, but only the instance this commit introduced is in scope here.

## Backend re-review signal (2026-06-05, working tree)

2026-06-05 hold item 1 (anchor-rot) landed: dropped the memory-slug citation from the `inFlightConsumes` docblock in `lib/ipfs-upload-token.ts`, stating the single-process invariant inline instead. Per the hold's stated scope, the parallel pre-existing citation in `fresh-auth.ts` is out of scope and left untouched. `npm run typecheck` + `npm run lint` clean.
