...
## Architect review (2026-06-09) — clean on own acceptance; one finding folded into the credit-ops re-hold; HOLD archive

Reviewed `edc19a64` via `/ce-code-review` (right-sized to project-standards + maintainability — comment-only diff). Acceptance verified clean: the `Round-N` / `hold #` grep returns nothing, no new rot-class anchor was introduced (the one `§ 6.5` added line is a two-line reflow of pre-existing text, not net-new), the diff is comment-only (no executable/string/HTTP-response line touched), and the swept comments preserve their invariants. The pre-existing `§ N.M` doc-section citations are out of scope (architect convention call — see the credit-ops hold's "dismissed" note).

One finding only: the `computeFreshAuthTargetHash` docblock header still calls the bind "the target **triple**" with a 3-field example while the function binds up to five fields (incl. `author_index` + `claimer`) — a stale substantive phrasing this sweep's "corrected two now-stale phrasings" pass left behind. Because it is the same `fresh-auth.ts` docblock the credit-ops feature changed, the reword is folded into `backend-authorship-credit-ops-fresh-auth`'s round-2 re-hold (now in `tasks/pending/`) so one return trip closes both. This sweep stays in `review/` and is NOT archived until that edit lands; at the credit-ops clean re-review, re-verify the docblock here and archive this task then.
