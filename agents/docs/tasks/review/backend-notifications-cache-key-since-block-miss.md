...
## Architect re-review (2026-06-08) — PARKED (round-2 resolved; not archived pending the cursor-ordering redesign)

`/ce-code-review` fan-out (correctness + adversarial on Opus; reliability, performance, testing, project-standards, kieran-typescript on Sonnet) on commit bed8734a. All three 2026-06-05 hold items are RESOLVED and verified: the internal fetch cap (`NOTIFICATION_WINDOW_FETCH_CAP=1000`) is decoupled from the response `limit`; `has_more` is recomputed as `filtered.length > events.length || batch.has_more` (no longer forced false on cursor removal); the call-count and clearVolatile-survival canaries discriminate. Cache mechanics (appTag prefix, stable-key survival, cross-account isolation) re-confirmed clean. Performance: the larger LIMIT does not change per-arm scan cost (the cap only bounds the final sort heap); the cache win is decisive.

NOT archived — one residual is the route-side half of a cross-task fetch-ordering issue, deferred (not held against this otherwise-resolved task):

- **Route-side >CAP window starvation + inert `has_more` (P2, bounded).** The window batch is fetched `ORDER BY block_num ASC LIMIT 1000` = the OLDEST 1000 events above the floor. For an account with >1000 in-window events (a popular author, or a victim an accredited attacker floods with cheap events), the newest events are never fetched; once the SPA cursor drains the cached batch it parks at the cap-top with `events=[]` + `has_more=true` (operationally inert — the SPA polls every 5 min and only advances when `latest_block > cursor`, so no tight-loop, but the bell feed silently truncates for that account). This is the route consumer of the same ASC-LIMIT root cause whose single-block (>response-limit) variant is tracked in `architect-notifications-block-granular-cursor-stall` (+ the `cursor-agnostic-cache-must-dominate-result-set` solution doc); the digest consumer is held in `backend-notifications-digest-window-cursor`. **Flag for whoever scopes the cursor redesign: confirm the >CAP window-starvation case (not only the single-block >limit stall) is in scope, or fold it in.** Pin the route `has_more=[]+true` contract before the already-implemented `ui-notifications-block-cursor-boundary-rewind` consumer relies further on it.

Minor items to fold when the redesign lands (none blocking alone): (a) the test header places the `MOCK_VERIFY_SIGNATURE` acknowledgment under clause (b), but CLAUDE.md requires it under clause (a); (b) the `lastNotificationParams[2] >= 500` assertion permits a silent cap regression to 500–999 — import the exported constant and assert equality; (c) a HAF timeout returns null and is not negatively-cached, so every TTL expiry after a timeout re-fires the full-cap fetch (a short tombstone TTL would dampen it); (d) the unsafe `as number` cast in the param-index assertion.

Disposition: parked in `review/` per triage. This task is archivable as round-2-resolved whenever the route residual's tracking in the cursor-redesign tasks is judged sufficient.
