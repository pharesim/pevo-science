...
`/ce-code-review` on commit `330b9012` (round-3 items 1-3; item 4 withdrawn). 9 reviewers — correctness, security, adversarial on Opus; testing, maintainability, project-standards, reliability, kieran-typescript on Sonnet; learnings-researcher unstructured; `ce-agent-native-reviewer` skipped per PEvO CLAUDE.md.

**Items 1 and 3 verified clean.** Item 1 reorder (`validateRetractParams` before `verifyHiveSignature` on `/retract`): correctness + reliability + security + adversarial all confirm the validator is pure (reads only `req.params.author`/`permlink`, no DB/HAF/Redis, side-effect-free), strictly cheaper than `verifyHiveSignature`, and introduces no auth-bypass and no existence-oracle info leak (the pre-auth 400 reflects only attacker-controlled slug shape, never paper/account existence). `retractLimiter` stays AFTER auth so `byAccount` keying still has `req.hiveUsername`; the handler's `req.hiveUsername!` invariant holds; both validator failure arms `sendError` + bare `return` with no `next()` fall-through. The custody siblings (`/upgrade`, `/fresh-auth`, `/session-auth`) correctly retain auth-first ordering — their limiters are `byAccount`-keyed — matching `agents/docs/solutions/conventions/validator-limiter-ordering-depends-on-key-class-2026-05-21.md`. Item 3 SSOT extraction verified complete: `grep -rn "PERMLINK_MAX_LEN\|PERMLINK_FORMAT_REGEX\|ROOT_PERMLINK_MAX_LEN" backend/src/` returns only the `hive-permlink.ts` definitions plus the `papers.ts`/`custody.ts` import sites; the shared constants are byte-identical to the removed locals. (`anonymousReview.ts` carries its own pre-existing inline `/^[a-z0-9-]+$/` + `256` check — out of item-3 scope; a candidate follow-up to adopt the shared module, not held here.)

Five items held. One P2 functional regression introduced by item 2 (3-way cross-reviewer corroboration), plus four coverage/hygiene items the user elected to bundle into this round.

### Items held (must fix before archive)

**1. (P2, anchor 100, cross-reviewer correctness + security + adversarial) Item 2's trim mutates the credential and breaks password byte-exactness on the custody re-auth routes.** `requireStringField` now returns `raw.trim()`. `/fresh-auth` and `/session-auth` consume the trimmed value (`const password = passwordResult.value`) and feed it to `argon2.verify(passwordHash, password)`. But every password-SET site (signup, set-password, recover) and the `/login` verify path operate on the RAW password, and `isPasswordValid` does not reject leading/trailing whitespace. Consequence: a user whose password carries edge whitespace can authenticate at `/login` (raw verify matches) but is rejected at `/fresh-auth` and `/session-auth` (trimmed verify fails) — locked out of consent broadcasts and critical-action re-auth; symmetrically, the two custody routes now accept a broader credential set than `/login`. This is a regression introduced by this commit — pre-commit, `requireStringField` returned the raw value, consistent with the set/login paths. Not an auth bypass (trimming the verify input can only reject a correct credential, never admit a wrong one), but a real functional + consistency defect.

  Fix (disposition: do NOT trim credential fields; implementer's call between two shapes): restore password byte-exactness so the bytes `argon2.verify` sees on `/fresh-auth` and `/session-auth` equal the bytes `argon2.hash` saw at signup/set-password/recover (matching the untrimmed `/login` path). Either (a) give `requireStringField` an opt-in trim parameter defaulting to no-trim, and pass trim=true only for the identifier/slug-shaped fields (`root_author`, `root_permlink`, `derived_pubkey`, `signed_proof`, `signed_at`), leaving `password` untrimmed; or (b) keep the helper's trim for the other fields but read the raw `password` for the `argon2.verify` input on both custody routes. The whitespace-only REJECTION for the non-credential fields (the original round-3 item-2 motivation) must be preserved by whichever shape is chosen.

**2. (P2, conf 90, cross-reviewer testing + correctness + kieran-typescript) `requireStringField`'s trim contract has zero direct unit coverage** despite being load-bearing across the seven `skipFailedRequests:true` adopters. Add a unit test for `requireStringField` (sibling to the other `backend/tests/lib/` specs) pinning: non-string input → reject; whitespace-only → reject; surrounding-whitespace input within the trimmed cap → success arm returns the TRIMMED value; cap-overshoot via padding → reject. After item 1's fix, also pin the credential byte-exactness invariant — the strongest form is a real-path test that sets a password with edge whitespace at signup, confirms `/login` accepts it, and asserts `/fresh-auth` + `/session-auth` accept the SAME byte-exact password.

**3. (P2, conf 90, testing; corroborated by reliability + correctness) The item-1 reorder — the load-bearing security behavior of this commit — has no test.** `papers-retract-url-shape-validator.test.ts` always sends mock auth headers, so the "validator fires before auth" guarantee (an UNAUTHENTICATED request with a structurally-invalid slug returns 400 VALIDATION_ERROR, not 401 UNAUTHORIZED) is unexercised. Add a spec sending a malformed slug with NO `X-Hive-Username` / signature headers and assert 400 (not 401). This pins the 400-before-401 ordering that is the stated security rationale for the reorder and catches a future re-reorder regression.

**4. (P3, conf 100, testing) Stale middleware-order comment.** `backend/tests/routes/papers-retract-real-path-verifyhivesignature.test.ts` documents the chain as `verifyHiveSignature → validateRetractParams → retractLimiter → handler` in two comment blocks; the reorder flipped it to `validateRetractParams → verifyHiveSignature → retractLimiter → handler`. Update both blocks to the current order. Anchor on the behavioral order; no line-number citations.

**5. (P3, conf 75, project-standards) Relative-positional anchor in the `/retract` route comment.** The route-registration comment ends "…documented on `validateRetractParams` above." Per `agents/docs/solutions/conventions/convention-enforcing-fix-must-audit-its-own-new-code-2026-05-17.md`, `above` is a relative-positional rot anchor. Rewrite to anchor on the named symbol, e.g. "…documented in the `validateRetractParams` JSDoc." (The primary anchor is already the function name, so the practical rot risk is low, but the convention is explicit and this is a one-word change in a file being touched for items 1/3/5 anyway.) Verify the replacement introduces no new rot class.
