...
Round-1 hold items #1-3 landed in `frontend/tests/unit/pages-settings.test.js`. Pure test improvements; no production code changed. Ready for architect round-2.

- **Item #1 (mnemonicToSeedSync timing spy):** the happy-path test `helper frame exits before _clearSensitiveUpgradeState runs (happy path)` now records `'mnemonicToSeed:call'` as a timed event whenever the mocked `mnemonicToSeedSync` fires. The assertion now requires the first in-helper-window seed call (filter: `i > enterIdx && i < exitIdx`) to fall strictly between `perform:enter` and `perform:exit`. A no-op `_performUpgradeKeyRotation = async () => {}` stub now fails the test: `firstSeedCallInHelper === -1`, fails `expect(...).toBeGreaterThan(enterIdx)`. The "find first IN HELPER WINDOW" filter is load-bearing — `_performKeychainImport` also calls `mnemonicToSeedSync` AFTER the broadcast helper exits, and a naive `events.indexOf('mnemonicToSeed:call')` would catch those legitimate post-helper calls too.
- **Item #2 (helper-internal broadcast rejection test):** new test `helper-internal broadcast rejection: catch wipes and frame pops before wipe` (added between the existing error-path test and `_performUpgradeKeyRotation returns undefined`). Stubs `vi.mocked(Client).mockImplementationOnce(() => ({ broadcast: { sendOperations: vi.fn(async () => { throw ... }) }}))` so the rejection happens INSIDE the helper, not after it. Uses a `try/finally` wrapper around `origPerform(...)` so `'perform:exit'` fires from the `finally` clause even when `origPerform` rejects — without that, the simple `await origPerform(...); events.push('perform:exit')` pattern from the happy-path test would skip the push on rejection and ordering would target the wrong event. Asserts: `upgradePhase === 'error'`, `enterIdx >= 0`, `exitIdx > enterIdx`, `wipeIdx > exitIdx`. Frame pop on rejection is now structurally enforced.
- **Item #2 (rename existing error-path test):** the existing `helper frame exits before _clearSensitiveUpgradeState runs (error path)` was renamed to `... runs (error path: post-helper fetch failure)` to surface that it exercises post-helper failure (`fetch` rejects after the helper has returned), not helper-internal failure. The test body is unchanged; the rename closes the architect's "mislabeled" callout without churning logic.
- **Item #3 (broadcast-without-Keychain test):** new test `broadcasts via sendOperations even when Keychain is uninstalled`. Pre-warms the next `Client` instance with a closure-captured `sendOpsSpy`, sets `mockIsKeychainInstalled.mockReturnValue(false)`, calls `_performUpgradeKeyRotation` directly (bypasses executeUpgrade so the assertion targets the helper's own invariant), asserts `sendOpsSpy` was called. Locks in: the broadcast (IRREVERSIBLE chain rotation) cannot be conditional on browser-extension state.
- **Imports:** added `import { Client } from '@hiveio/dhive';` (immediately after the existing `mnemonicToSeedSync` import) so items #2 and #3 can `vi.mocked(Client).mockImplementationOnce(...)`.
- Verified: `npx vitest run tests/unit/pages-settings.test.js -t 'FE-UPGRADE-CLOSURE-WIPE'` → 6/6 (was 4, +2 new tests); full pages-settings file → 51/51; full frontend unit suite → 1028/1028.
