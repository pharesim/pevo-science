...
## Backend landing note (2026-06-09, commit `77571323`)

**Sub-goal 1 (DONE):** appended the `block_num DESC, cj.id DESC` same-block tie-breaker to the four missed accreditation-state reads — `findAccreditedAccountWithOrcid` (first read + binding-live re-check) and `getExistingAccreditation` in `routes/orcid.ts`, and `getAccreditationFromHaf` in `routes/profile.ts` — each with the convention-path anchor comment (referenced by behavior, not a dated filename). `window-cte-deterministic-tiebreaker.test.ts` green (3); `orcid.test.ts` (102) + `profile-auth-bypass.test.ts` green against current main. typecheck + lint clean.

**Sub-goal 3 (DONE, via the note-extension option):** updated the in-test coverage note to record these as inline reads (not exported fragments → not canary-inspectable; guarded by route integration tests instead).

**Sub-goal 2 (NOT IMPLEMENTED — architect decision needed):** the single-source ranking-fragment extraction (so the shape canary reaches `getAccreditedSet` / the accreditations-route inlined sites / the orcid+profile reads and guards against tie-breaker removal) was NOT done — it was outside the scope the implementer was briefed on. The four sites are correct and deterministic now, but a future edit dropping `, cj.id DESC` from any inline site would still pass the suite (the live corpus is unlikely to contain a same-block accredit/revoke pair), which is the canary-reachability gap the GOAL names. The gap intersects both the dismiss-preemptive-test-hardening posture (failure mode is theoretical on the live corpus) AND the hot-path-query-plan caution this task itself flags for `getAccreditedSet`. Architect to decide: accept the documented-inline approach (close), or hold for the narrower `ORDER BY`-constant extraction + canary assertion. Flagging rather than silently choosing.
