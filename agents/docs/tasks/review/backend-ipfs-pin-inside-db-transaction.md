...
Both minor items landed.

- **rowCount null-guard at the row-absence re-check.** `rowAbsent = check.rowCount === 0` → `rowAbsent = check.rowCount !== null && check.rowCount === 0`, matching the file's two sibling `rowCount` checks. Behavior is unchanged (`null === 0` was already `false` → skip unpin), but the explicit guard makes the conservative bias legible; an in-code comment notes that a null `rowCount` is not proof of absence and must not green-light the unpin.
- **`unpinFromPinata` benign-absence tolerance now tested.** New unit `tests/lib/ipfs-shared-unpin.test.ts` stubs the global `fetch` and asserts: a non-2xx whose body carries the not-pinned signal (`CURRENT_USER_HAS_NOT_PINNED_CID`, `not_pinned`, spaced `not pinned`, uppercase `NOT PINNED`) resolves without throwing; a clean 2xx resolves; a non-benign non-2xx (`500 Internal Server Error`) still throws `Pinata unpin failed: 500`. This pins the content-based tolerance branch the pre-fix code lacked, so a revert to unconditional-throw fails red. Per the adversarial note, the match is content-based (not status-gated) because the live already-unpinned Pinata body is undocumented (docs.pinata.cloud documents only the 200); the test pins current behavior rather than over-fitting a verbatim string. Carve-out clauses (a)/(c) are documented in the test header — `fetch` is the only mock, no auth/crypto is involved, and the integrated companion is the Pinata compensation-dispatch test in the pin-durability route suite (real route + config toggle, success path).

**Verification.** `npm run typecheck` clean (src + tests); `eslint` clean on `routes/ipfs.ts` + the new unit test; scoped `npx vitest run tests/lib/ipfs-shared-unpin.test.ts tests/routes/ipfs-pin-durability.test.ts` → 13/13 green against real Redis (the pin-durability suite's logged 500/503 lines are its intentional failure-injection scenarios). Self-audit on changed lines: no task-slug citations, round-N markers, line-number anchors, SHA refs, date anchors, or relative positional anchors in the source/test files.
