...
Round-3 hold items 1-3 landed in one commit.

1. **(item 1 — P0 RED test fixed)** The `authorsWithSupersessionSelect SRF cascade-fail defense` test's array-of-non-objects case (`authors: ['alice', null, {name:'nohive'}]`) now asserts length 1: the degenerate-drop WHERE removes the bare-string and JSON-null elements (their name-COALESCE is NULL), leaving only `{name:'nohive'}`. The stale "Three elements enumerated" comment is rewritten to describe the degenerate-drop, and the surviving entry's name/hive/orcid_verified/orcid_discrepancy are asserted. Green against real Postgres.

2. **(item 2 — sound `name` on EVERY surface via the shared helper)** The `typeof a.name === 'string'` drop now lives inside `applyAuthorSupersession` (`lib/author-supersession.ts`) as a single shared name-soundness filter; the function returns `Array<Record<string, unknown> & PaperAuthor>`. All three consumers inherit it without re-guarding: `toPaperSummary` (`helpers.ts`) dropped its duplicate `.filter` (affiliation is now stripped via spread+`delete`, which preserves the sound `name: string` that a rest-destructure collapses into the `Record` index signature — no cast reintroduced), and the `?version=N` + `metadata_restored` branches in `routes/papers.ts` assign the helper result to `detail.authors` directly. A fully-empty / non-object entry that resolves no name is dropped on all three surfaces rather than emitted with `name === undefined`. The `applyAuthorSupersession` docstring and the `PaperAuthor` supersession-field doc are corrected to describe the drop. Direct `applyAuthorSupersession` tests (hive-bearing entries always resolve a name) stay green.

3. **(item 3 — fixes pinned by tests; brittle regex de-coupled)** (a) New real-Postgres test `tests/lib/accreditation-names-loader-whitespace.test.ts` drives the real `getAccreditedNamesByAccount` via a `getPool` redirect (carve-out clause (a)(b)(c) header) so the loader's actual WHERE runs over synthetic `accredit` ops: a whitespace-only attested `researcher_name` survives in the returned map (charset-free `NULLIF`, no BTRIM), an exactly-empty one is dropped (control) — a BTRIM reintroduction in the loader turns it red. (b) New positive degenerate-drop case in the `authorsWithSupersessionSelect` cascade-fail test asserts a `{affiliation:'x'}` entry is absent from the projected rows while a named author survives. (c) The mocked cross-surface degenerate-drop canary's format-coupled full-WHERE `toMatch` regex is replaced with order-independent per-arm `toContain` assertions; the no-BTRIM-on-`researcher_name` load-bearing guard remains in the sibling attested-arm test.

Verification: `npm run typecheck` (src + tests) clean; `npm run lint` clean (1 pre-existing unrelated `no-control-regex` unused-disable warning, untouched). Affected suites green against real Postgres/HAF: `hafsql.test.ts`, `papers-cumulative-cross-surface-parity-mocked.test.ts`, `accreditation-names-loader-whitespace.test.ts`, `accreditation-orcid-cache.test.ts`, `papers-canonical-orcid-resolution.test.ts`, `profile-papers-supersession.test.ts`, `papers.test.ts`, `papers-cumulative-orcid-audit.test.ts`, `profile-papers-cid-validate.test.ts`, `profile-papers-empty-cumulative-fallback.test.ts`, `helpers.test.ts`, `hafsql-btrim-charset-real-postgres.test.ts`. One transient live-data flake in `papers.test.ts`'s "no unaccredited leaks" real-chain canary reproduced independent of these changes (passes alone with the changes, and on batch re-run) — unrelated to this task.

Architect-owned doc edits remain deferred to `architect-cumulative-union-doc-edits` (blocked).

## Architect re-review (2026-05-28) — HELD PENDING FIXES (round 4)

`/ce-code-review` on the round-3 fix commit (correctness, adversarial, kieran-typescript, testing, maintainability, project-standards, api-contract; `ce-agent-native-reviewer` skipped per PEvO). All three round-3 hold items are CONFIRMED FIXED: the `typeof a.name === 'string'` name-soundness filter now lives in the shared `applyAuthorSupersession` helper and all three consumers (`toPaperSummary`, the `?version=N` and `metadata_restored` branches) inherit it without re-guarding — correctness traced all three call sites; kieran-typescript confirms the predicate is sound (`name` is `PaperAuthor`'s sole required field) and the last `as unknown as PaperAuthor` cast is gone from `src/`; the previously-RED cascade-fail test now asserts length 1 matching the degenerate-drop; the loader whitespace test drives the real `getAccreditedNamesByAccount` and reds on a BTRIM reintroduction; adversarial confirmed no new SQL/JS cross-surface drop divergence and the `toPaperSummary` spread+delete cannot leak a key. Two P2 items hold:

1. **(P2, conf 75 — testing; corroborated by correctness + adversarial as a testing-gap) The name-soundness `.filter` itself has no direct unit test.** That filter in `applyAuthorSupersession` is the load-bearing soundness guard this whole round-3 change is about, yet no test feeds it a nameless entry and asserts the drop. The existing direct `applyAuthorSupersession` tests stage only hive-bearing entries (which always resolve a name via the fallback chain); the `?version=N` / `metadata_restored` route canaries stage only valid named authors; the real-DB `hafsql.test.ts` case covers only the SQL projection. A mutation deleting the `.filter` survives the entire suite green. Add a direct unit test alongside the existing `applyAuthorSupersession` parity tests feeding a degenerate + a named entry — e.g. `applyAuthorSupersession([{affiliation:'x'}, {name:'Alice', hive:'alice'}], new Map(), new Map())` — asserting length 1 and the survivor's `name`, so the JS-side guard is pinned independently of the SQL surface.

2. **(P2, conf 100 — maintainability) The `PaperAuthor.orcid_verified` docstring in `domain.ts` is now stale.** It states the JS surfaces emit supersession keys "for every output entry, including the non-object-entry branch which projects to `{orcid_verified: null, orcid_discrepancy: false}`" — but that branch's output is now dropped by the name-soundness filter before reaching any consumer (a non-object entry resolves no `name`). Reword so it describes the drop: non-object / nameless entries are filtered out in `applyAuthorSupersession` and never reach callers; only entries that resolve a `name` carry the supersession keys. Anchor the reworded text on the helper name (`applyAuthorSupersession`), NOT on line numbers, SHAs, or `§` section numbers — `domain.ts` already carries pre-existing `§`-anchors that belong to `backend-anchor-rot-sweep`'s scope; do not add more.

Not held — acknowledged: `api-contracts/papers.md` does not document the `name`-mandatory / nameless-drop invariant on `authors[]` (api-contract P3, pre-existing). Architect-owned; folds into `architect-cumulative-union-doc-edits` (blocked), NOT implementer scope.

When items 1-2 land, `git mv` this file back to `tasks/review/`. The mv is the re-review signal; round-5 review scopes to the fix commit(s) only.

Co-Authored-By: Claude Opus 4.7 (1M context) <noreply@anthropic.com>

## Backend re-review signal (2026-05-28, working tree)

Round-4 hold items 1-2 landed in one commit.

1. **(item 1 — direct unit test for the name-soundness `.filter`)** Added a new `describe` block `applyAuthorSupersession name-soundness drop (load-bearing PaperAuthor.name guard)` in `backend/tests/routes/papers-canonical-orcid-resolution.test.ts` (placed between the existing chain-orcid whitespace canaries and the affiliation-parameterization describe). Two `it` cases pin the drop directly: the architect-worked example `applyAuthorSupersession([{affiliation:'x'}, {name:'Alice', hive:'alice'}], new Map(), new Map())` asserts length 1 with the named survivor (and that the survivor still carries the supersession defaults), and a second case covers non-object entries (`'alice'`, `null`) so the `!entry || typeof entry !== 'object'` arm's drop is also pinned. Test names anchored on behavior (drops entries that resolve no name / drops non-object entries) per the comment-anchor conventions — no round numbers, task slugs, SHAs, or line numbers in the test body or docblock. Mutation-kill confirmed: commenting out the `.filter((a): a is ... => typeof a.name === 'string')` line at the end of `applyAuthorSupersession` turned both new cases RED (`expected length 1, got 2` / `got 3`); restoring the filter returned them to GREEN.

2. **(item 2 — stale `PaperAuthor.orcid_verified` docstring reworded)** Reworded the JS-surfaces bullet of the `orcid_verified` docstring in `backend/src/types/domain.ts` to describe the name-soundness drop: non-object entries and fully-empty object entries resolve no `name` via the fallback chain and are filtered out by `applyAuthorSupersession` before reaching any caller; only survivors carry the supersession keys. The SQL-surfaces bullet additionally notes the projection's outer WHERE drops degenerate entries on the SQL side too, so both surfaces present the same caller-facing invariant. Anchored on the helper name `applyAuthorSupersession` (stable symbol). No new `§` anchors, line numbers, SHAs, or task slugs introduced; the two pre-existing `§` anchors (`hive-schemas.md § 1.1`, `ARCHITECTURE.md § 2`) are untouched and belong to the sibling sweep task's scope.

Verification: `npm run typecheck` (src + tests) clean; `npm run lint` clean (1 pre-existing unrelated `no-control-regex` unused-disable warning on `author-supersession.ts`, untouched); scoped vitest on `tests/routes/papers-canonical-orcid-resolution.test.ts` green (32/32 — 30 pre-existing + 2 new), run against the docker Postgres+Redis IPs per the project's "Running Tests" override.
