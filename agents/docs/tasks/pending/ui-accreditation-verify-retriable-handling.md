...
UI re-review signal (2026-05-17, working tree, commit pending): round-2 hold items landed.

- Item 1 (concurrent `_verify()` race, `_verifyGeneration` counter): `frontend/src/pages/accreditation-verify.js` — added `_verifyGeneration: 0` property mirroring the existing `_cooldownId` field, bumped synchronously at the top of `_verify()` (`const generation = ++this._verifyGeneration;`), and captured into both `.then` and `.catch` closures with `if (generation !== this._verifyGeneration) return;` bail-out positioned after `_mounted` checks but before any state write. Pattern matches `agents/docs/solutions/conventions/synchronous-flag-before-await-idempotency-guard-2026-05-16.md`.
- Item 2 (`retryVerification` double-submit window, loading-state guard): `frontend/src/pages/accreditation-verify.js` — added `if (this.state === 'loading') return;` as the first guard line in `retryVerification()`, ahead of the existing `retryCooldownRemaining > 0` check. Belt+suspenders with item 1's generation counter as the comment block documents.
- Item 3 (test coverage for both guards): `frontend/tests/unit/pages-accreditation-verify.test.js` — added two specs inside the existing `describe('retriable error handling')` block. First spec dispatches flight A (manual-resolve), then flight B (rejection), waits for B's `retriable_error` state, then resolves A and asserts `state` stays `retriable_error` and `resultUsername` stays `''` — A's late `.then` bails on generation mismatch. Second spec drives Retry double-tap by triggering `retryVerification()` three times while flight 2 holds a never-resolving promise; asserts `mockVerifyAccreditation` is called exactly twice total (init + first valid retry). All 13 specs pass under `npx vitest run tests/unit/pages-accreditation-verify.test.js` (4 pre-round-1 + 7 round-1 retriable + 2 new round-2 race).

---

Architect re-review (2026-05-17, round-3) — HELD PENDING FIXES:

Round-2 implementation at commit `cce7b41` correctly landed all three round-2 hold items. The race-condition guards are mechanically sound under `/ce-code-review` cluster (correctness, testing, maintainability, project-standards, julik-frontend-races, reliability, learnings-researcher): bump-before-dispatch ordering is synchronous and same-tick-safe, closures capture `generation` by value, `_mounted` then generation bail-out order is correct, no half-set state on bail, no stuck-loading regression, no cross-mount generation leak (factory pattern returns a fresh object per mount — confirmed against `Alpine.data()` reuse model and against the `alpine-init-handler-deregister-before-reassign-2026-05-17` learning). One item blocks archive.

1. **Inverted preamble comment on the concurrent-flights spec** (P3 — cross-reviewer: testing + julik-frontend-races, anchor 100).
   `frontend/tests/unit/pages-accreditation-verify.test.js:~258` — the spec's preamble comment ends: *"The user-facing failure shape this closes: first verify resolves with success AFTER the second verify overwrites with retriable_error — the success state must survive."* The final clause is exactly backwards. Flight B (the retriable_error rejection) is the LATEST flight and flight A (the success resolve-after-B) is the STALE one; the generation guard exists so flight B's `retriable_error` survives, not flight A's late success. The assertion at the spec's tail (`expect(comp.state).toBe('retriable_error')` and `expect(comp.resultUsername).toBe('')`) is mechanically correct — only the prose is inverted. A future maintainer who reads "the success state must survive" and then tries to "fix" the assertion the wrong way silently reverses the guard.

   **Fix shape:** trim/rewrite the inversion only. Replace "*the success state must survive*" with something like "*flight B's retriable_error state must not be overwritten by flight A's stale success resolution; the generation guard makes flight A's late `.then` bail.*" The assertions themselves do not change. Pure prose, 1 line.

Findings triaged dismissed this pass:
- reliability REL-1 (P3/75): `destroy()` does not reset `retryCooldownRemaining`, hypothetical leak if Alpine remount reuses the same instance. **Dismissed** — the registered factory (`createComponent()` / `Alpine.data()` pattern) returns a fresh object literal each mount; `retryCooldownRemaining` is reinitialized to `0` per mount, so the leak is unreachable. The cross-reviewer pass independently confirmed this (julik-frontend-races RR-05). Per project memory `feedback_dismiss_preemptive_test_hardening.md`, default-dismiss preemptive hardening.
