...
## Architect re-review (2026-05-26) — round-3 fix VERIFIED; HELD PENDING FIXES (round 4)

`/ce-code-review` on the round-3 per-tier-epoch commit dispatched 9 reviewers (correctness + adversarial on Opus; testing/maintainability/project-standards/performance/reliability/kieran-typescript/learnings on Sonnet; `ce-agent-native-reviewer` skipped per PEvO root CLAUDE.md). **The round-3 P1 hold is verified resolved.** Both Opus reviewers traced all four tier×invalidation combinations (stable+clearVolatile writes; non-stable+clearVolatile suppressed; stable+invalidate suppressed; non-stable+invalidate suppressed) and found no defect; performance confirmed the stable-cache-warmth regression is genuinely closed; project-standards found no comment-anchor violations in the new docblocks. An adversarial/kieran residual ("`stableKeys` is empty under the Redis backend, so `clearVolatile` would delete stable Redis keys") was checked and **dismissed as a false positive** — `set()` runs `this.stableKeys.add(key)` BEFORE the Redis early-return, so stable-key membership is populated regardless of backend.

One test-coverage item is held below. Two pre-existing reliability/perf items were split into a separate task (`backend-cache-keys-scan-and-invalidateprefix-race`) and are NOT part of this hold.

### Item to address

**1. (P3, cross-reviewer: correctness + adversarial + testing + kieran-typescript) The two converse per-tier suppression directions are unexercised — a regression that drops a counter bump or mis-wires the non-stable gate would pass the suite.**

The round-3 spec pins the one direction it needed (`clearVolatile()` during an in-flight STABLE fetcher does NOT suppress the write). The two converse directions — where a write MUST be suppressed — are only covered transitively, so the per-tier gate is under-pinned:

- **(a) stable in-flight fetcher + `invalidate()`/`clear()` → write MUST be suppressed.** This is the reason `invalidate()`/`invalidatePrefix()`/`clear()` bump `stableEpoch` (not only `volatileEpoch`). No spec isolates it: a mutation dropping `this.stableEpoch++` from `invalidate()`/`clear()` re-caches a stale stable snapshot and the suite stays green.
- **(b) non-stable in-flight fetcher + `clearVolatile()` → write MUST be suppressed.** This is the reason the non-stable gate keeps the `capturedVolatileEpoch === this.volatileEpoch` conjunct. No spec isolates it: a mutation collapsing the non-stable branch to read only `stableEpoch` re-caches a stale volatile snapshot, and the round-2 `invalidate()` spec cannot catch it (that spec bumps BOTH counters, so suppression there can't be attributed to the volatile conjunct).

**Fix:** add two unit specs in `backend/tests/lib/cache.test.ts`, mirroring the existing in-flight-race pattern (slow fetcher; short delay so the in-flight slot registers; trigger the invalidation; then assert the in-flight caller still receives the value AND `cache.get(key)` is `undefined` after resolution):
- spec (a): in-flight `stable: true` fetcher + `invalidate(key)`. Mutation-kill: remove `this.stableEpoch++` from `invalidate()` → RED.
- spec (b): in-flight non-stable fetcher + `clearVolatile()`. Mutation-kill: drop the `volatileEpoch` conjunct from the non-stable branch → RED.

Anchor the new spec comments on the behavior (which suppression direction, and the mutation each kills) — no round numbers, line numbers, or SHAs in the test source.

### Items NOT held (recorded for transparency)

- **Pre-existing blocking `redis.keys()`** in `clearVolatile()`/`clear()` (vs the non-blocking `SCAN` that `invalidatePrefix` already uses), and the **`invalidatePrefix` mid-`SCAN` epoch-bump window** (a fetcher starting between SCAN batches captures post-bump epochs and could write to a not-yet-deleted key) — both pre-existing, theoretical at single-instance scale, split into `backend-cache-keys-scan-and-invalidateprefix-race` in `tasks/pending/`.
- **`notInvalidated` ternary clarity** (maintainability/kieran, conf 40–50): below action threshold — the docblock already explains why the stable branch omits `volatileEpoch`. No action.

### Carry-forward for archive (architect-owned, after round-4 lands)

- `agents/docs/solutions/conventions/single-flight-coalescing-amplifies-cache-invalidation-race-2026-05-20.md` is now stale (its examples show a single `epoch`; HEAD has per-tier counters). Refresh it — and capture the per-tier-epoch learning via `/ce-compound` — at archive time, once the per-tier shape is final.
