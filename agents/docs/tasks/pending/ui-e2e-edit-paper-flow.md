...
2. **P3 — `comment_options` op assertion comment misstates which field enforces the no-Hive-rewards principle (`frontend/tests/e2e/edit-paper.spec.js:557`).** Cross-reviewer agreement: correctness (P3/90) + testing (P3/90). The comment introduced by round-2 item 3 says "PEvO no-Hive-rewards principle is enforced via `comment_options.allow_curation_rewards: false`; a regression dropping this op enables curation rewards on continuations". Production at `edit.js:1004` actually sets `allow_curation_rewards: true`; the no-rewards principle is enforced via `max_accepted_payout`. The presence assertion itself (`expect(optionsOp).toBeTruthy()`) is fine as a regression-guard for the op being broadcast at all. Fix: rewrite the comment to name `max_accepted_payout` as the load-bearing field. Optionally tighten the assertion to also pin the production value of `max_accepted_payout` (read `optionsOp[1].max_accepted_payout` and assert it equals the production value — read `frontend/src/pages/edit.js:1000-1010` for the canonical shape). The tightened assertion makes the test a true mutation-kill for the no-rewards regression class, not just a presence pin.
