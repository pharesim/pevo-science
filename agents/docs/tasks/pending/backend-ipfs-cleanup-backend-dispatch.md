...
Closed the held domain-enforcement item across all three defense layers (DB CHECK + read-time narrowing + exhaustive dispatch + regression test). The out-of-domain `pin_backend` value can no longer reach the silent Pinata-fallthrough-then-reap path.

- **(DB) CHECK constraint on migration 014.** Edited the existing `014_pending_ipfs_uploads_pin_backend.sql` (not shipped to prod) to add `CHECK (pin_backend IN ('kubo','pinata'))` after the column add, following the `state`-discriminator CHECK precedent in `010_bridge_import_queue.sql`. Made idempotent via `ALTER TABLE … DROP CONSTRAINT IF EXISTS pending_ipfs_uploads_pin_backend_check;` then `ALTER TABLE … ADD CONSTRAINT … CHECK (…)` so re-applying to the dev DB (where the column already exists from the prior run) does not error on a duplicate constraint. The `schema_migrations` self-record INSERT (`ON CONFLICT DO UPDATE`) is unchanged.
- **(code) read-time narrowing.** Added `toPinBackend(value: string): PinBackend` to `lib/ipfs-shared.ts` — throws `Unrecognized pin backend: …` on anything outside the union. `runCleanup` now calls `unpinFromIpfs(row.cid, toPinBackend(row.pin_backend))` instead of the bare `row.pin_backend as PinBackend` cast; the throw is evaluated as the argument (before the unpin fetch and before the DELETE), so `runCleanup`'s existing per-row `try/catch` logs `IPFS cleanup error processing CID` and skips the reap, leaving the row for an operator. Removed the now-unused `PinBackend` type import from `ipfs-cleanup.ts`.
- **(code) exhaustive dispatch.** Rewrote `unpinFromIpfs` from `if (kubo) … ; return unpinFromPinata` to a `switch` with explicit `'kubo'` and `'pinata'` cases plus a `default` that binds `const unreachable: never = backend` and throws — so a future union widening without a matching branch fails to compile (the `never` assignment) rather than silently routing to Pinata.
- **(test) unrecognized-value regression.** Added a third case to `tests/ipfs-cleanup-backend-dispatch.test.ts`: seeds an orphan with `pin_backend='s3'`, drives the real `runCleanup`, and asserts zero unpin fetches (no Kubo `pin/rm`, no Pinata `unpin`) and zero `DELETE FROM pending_ipfs_uploads` calls. A regression back to the silent-fallthrough shape would issue a Pinata DELETE + reap the row and red this case. Carve-out clauses (a)/(b)/(c) in the file header already cover the new case (same mocked app pool / HAF / IPFS-client justification); no header change needed.

**Verification** (worktree fast-forwarded to current `main` first; its base was stale and lacked the landed cluster work + this task file):
- `npm run typecheck` clean (src + tests).
- `eslint src/lib/ipfs-shared.ts src/ipfs-cleanup.ts tests/ipfs-cleanup-backend-dispatch.test.ts` clean (exit 0).
- `npx vitest run tests/ipfs-cleanup-backend-dispatch.test.ts` → 3/3 green; logs confirm `pinata`→Pinata unpin, `kubo`→Kubo `pin/rm`, `s3`→`toPinBackend` throw caught by `runCleanup` (logged + skipped, no DELETE).
- Migration applied to dev `pevo_app` twice in a row, both exit 0 (idempotent: second run's `DROP CONSTRAINT IF EXISTS` drops then re-adds without error). Constraint verified present (`CHECK ((pin_backend = ANY (ARRAY['kubo'::text, 'pinata'::text]))`); a direct `INSERT … pin_backend='s3'` is rejected with a constraint violation, `'pinata'` accepted (test rows cleaned up).
- Self-audit on changed lines: no task-slug citations, round-N markers, line-number anchors, SHA refs, or date anchors in source/test; the architect hold block is unedited.
