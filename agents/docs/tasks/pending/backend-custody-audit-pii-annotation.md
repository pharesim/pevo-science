...
## Architect re-review (2026-05-16, round-4 → round-5) — HELD PENDING FIXES

`/ce-code-review` ran on commit `500b86d` (6 reviewers: correctness on Opus; adversarial on Opus; maintainability, project-standards, learnings-researcher, data-migrations on Sonnet; `ce-agent-native-reviewer` skipped per project CLAUDE.md). All three round-3 hold items land cleanly (verified: scope sentence rewritten, route-policy assertion + slug parenthetical dropped at both occurrences, `e.g.,` form applied at both occurrences). One item held; two items dismissed at triage.

### Items held (must fix before archive)

**1. (P2, conf 75, adversarial) Column-comment scope is "broadcast-only" but `custody_audit_log` carries non-broadcast rows that always write `user_agent` NULL — CNPD inspector reads a contradiction.** `backend/migrations/006_custody_audit_pii_annotation.sql:43-58` (the `COMMENT ON COLUMN user_agent` body) describes the column as populated whenever a fresh-auth challenge is answered for a broadcast (POST /api/custody/broadcast). However, `custody_audit_log` has 11 INSERT call sites across the codebase via `logCustodyBroadcast` / `logCustodyAudit`; the broadcast path is just 2-3 of them. The remaining sites (`login_failure`, `password_reset`, `account_recovery`, `recovery_failure`, `email_deleted`, `upgrade`, `upgrade_failure`) ALL write `user_agent` as NULL (they call `logCustodyBroadcast` without `auditExtras`, or use a different helper that doesn't pass UA).

  A CNPD inspector running `\d+ custody_audit_log` reads the COMMENT and forms the mental model "this column is populated for broadcast rows." The inspector then sees the table content: most rows have `user_agent IS NULL`. Two interpretations: (a) "the broadcast paths aren't populating it correctly — GDPR breach," or (b) "the COMMENT is wrong — there are many non-broadcast row classes." The inspector cannot tell which from the COMMENT alone.

  **GDPR/CNPD audit-trail impact**: this is the exact failure mode round-2/3/4 holds were designed to prevent — a contradictory or scope-incomplete COMMENT that an inspector relies on. Round-4's "answer a fresh-auth challenge" wording is correct for the broadcast path but doesn't acknowledge that the COLUMN exists on a table with many other row classes.

  Fix shape (architect call between two options):
  - **Option A — narrow the COMMENT scope to "when populated":** rewrite the opening sentence to "Raw HTTP User-Agent header, populated ONLY for rows generated by POST /api/custody/broadcast (vote, comment, custom_json, author_accept, author_resign). Other row types in this table (login_failure, password_reset, account_recovery, upgrade, etc.) write this column as NULL." This is precise but couples to the current op-type table; future row types added without UA capture won't trigger any signal that the COMMENT needs an update.
  - **Option B — broaden to the universal column semantic:** "Raw HTTP User-Agent header captured by the broadcast handler when a fresh-auth challenge is answered. Other row types in this table (non-broadcast custody-audit events: login_failure, password_reset, account_recovery, upgrade, etc.) write this column as NULL by default — they do not run the fresh-auth gate, so no UA capture occurs. Therefore: NULL on this column means either (a) the row is non-broadcast, or (b) the HTTP client did not send a User-Agent header on a broadcast call." This explicitly names the two NULL causes; a future row type with UA capture would need a COMMENT update but won't silently rot.

  Architect recommendation: Option B. It names the column's two NULL paths explicitly, gives the inspector unambiguous interpretation, and survives future op-type additions because the "non-broadcast" framing is durable.

  Apply to BOTH occurrences (SQL header comment block + `COMMENT ON COLUMN` body); preserve the existing retention / legal-basis / right-to-erasure / kind-classifier paragraphs unchanged. Migration remains idempotent (in-place edit OK; no migration 007 needed).

### Items dismissed during architect triage

- **(P2, conf 75, adversarial) Boot-fatal regex parse of column COMMENT is a hidden coupling surface for future word-level edits.** Adversarial concern: a future migration's COMMENT edit (e.g., adding a second "Retention: N months" substring, or changing wording to "Retention period:") silently shrinks/breaks the GDPR window. **Dismissed because the fail-loud-on-mismatch behavior is the intended design** — that's exactly why the round-2 hold tightened the regex and the convention `load-bearing-greps-at-signal-block-write-time-2026-05-06.md` formalizes the grep verification at signal-block-write-time. A future edit that breaks the regex causes a boot-fatal `BootFatalError`, which is the right operator signal (loud, deploy-blocking). Defense against accidental wording drift lives in the verification gate convention, not in the migration text.
- **(P3, conf 75, adversarial) Migration re-run order silently overrides any future migration that touches the user_agent COMMENT — 006 always wins.** True observation about `./deploy.sh migrate`'s re-run-all behavior, but **this is the deliberate design**: `COMMENT ON COLUMN` is unconditional and overwrites; migration 006 IS the canonical COMMENT source. A future migration 007 that wants to change the COMMENT must edit 006 in place (as the round-3/4 holds did) rather than add a competing 007 statement. The "006 always wins on re-run" is a feature for idempotent comment management, not a bug. The migration's own header already documents this.
- **(P3, conf 85, adversarial) Post-boot COMMENT corruption (manual DBA edit, dropped comment) silently disables retention enforcement until operator notices error log.** Risk requires deliberate DBA action (operator running `COMMENT ON COLUMN ... IS NULL` manually). Per memory `feedback_pevo_logging_minimal`, the existing error-level tick log is the right signal; adding higher-severity alerting (re-throw BootFatalError on COMMENT corruption mid-runtime) would over-engineer for a manual-misconfiguration scenario. Accepted as documented residual.
