...
All 3 round-3 hold items landed in a single commit on `frontend/tests/e2e/coauthor-accredited-prefill.spec.js`.

- Item 1 (P1, delete one-call wrapper). Removed the `pickTwoAccreditedResearchersWithOrcid` file-scope wrapper. Inlined the `pickAccreditedResearchers(request, { count: 2, limit: 20, predicate: (r) => !!r.username && !!r.orcid })` call at all 3 call sites (now at ~lines 146, 217, 331 in the publish-UI, publish-broadcast, and edit-form tests respectively). The carved-out predicate explanation comment rides above the first call site (~lines 139-145); the other two call sites carry the call without re-explaining since the rationale is now on its first occurrence.
- Item 2 (P1, hoist contract probe to beforeAll). Added a `test.beforeAll(async ({ request }) => { await assertAccreditationsListContract(request); });` hook at ~line 127, immediately after the `ACCREDITED_BADGE_TEXT` constant and before the first `test(...)` block. Removed the 3 per-test `await assertAccreditationsListContract(request)` calls. The helper definition at ~lines 49-60 stays as-is. Net effect: one HAF round-trip per spec run (vs three previously). Verified via `playwright test --list` — structural test count is unchanged at 3.
- Item 3 (P2, raise waitForAccreditedDirectoryLoaded timeout). Raised the `waitForFunction` timeout from `{ timeout: 10_000 }` to `{ timeout: 20_000 }` at ~line 94. The wrap-and-rethrow catch block and its error message are unchanged. Extended the comment above the helper (~lines 73-84) to document the rationale: cold-HAF tail latency on CI runners (window-function CTE over `custom_json` since genesis on cold cache, plus scheduler contention plus the Alpine reactivity tick).

Sanity check: `npx playwright test --list tests/e2e/coauthor-accredited-prefill.spec.js` shows the same 3 tests at the expected locations (135, 213, 327). `node --check` passes.

Test status: Playwright deferred to parent agent per UI agent CLAUDE.md (E2E specs share the dev backend port; concurrent runs collide).

NOT in scope (dismissed at architect triage round-3): JFR-002 `installPaperMocks` suffix exclusion list (preemptive), JFR-004 `.nth(rowIdx)` Alpine re-sort defense (preemptive), JFR-003 cross-request-context cache (info-only), `assertAccreditationsListContract` envelope-vs-rows-empty conflation and `pickAccreditedResearchers` HAF row shortage (data-dependent theoretical).
