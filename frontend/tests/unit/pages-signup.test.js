...
describe('signupPage', () => {
  ...
  describe('handleOrcidVerify', () => {
    ...
    it('rejects a non-allowlisted redirect host without navigating', async () => {
      ...
      const warned = warnSpy.mock.calls.find((c) => c[0] === '[signup orcid verify]');
  ...
  describe('handleOrcidSignup', () => {
    ...
    it('rejects a non-allowlisted redirect host without navigating', async () => {
      ...
      expect(comp.orcidLoading).toBe(false);
      expect(sessionStorage.removeItem).toHaveBeenCalledWith('pevo_orcid_mode');
      const warned = warnSpy.mock.calls.find((c) => c[0] === '[signup orcid start]');
      expect(warned).toBeDefined();
      expect(warned[1].message).toBe('Invalid ORCID redirect URL');
      warnSpy.mockRestore();
    });
  ...
  // The ORCID redirect handlers set orcidLoading=true then navigate away,
  // resetting only in catch. On browser Back the page restores from bfcache
  // (init()/destroy() do not re-run) and would stay disabled on its loading
  // label. A persisted pageshow (bfcache restore) must reset the flag.
  // bfcache cannot be simulated in jsdom, so a listener-recording window stub
  // lets us fire a synthetic pageshow whose `persisted` we set explicitly.
