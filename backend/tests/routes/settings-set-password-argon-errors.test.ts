 ...
 * `POST /api/settings/set-password` (settings.ts:352). Closes the route-level
 ...
 * settings.ts:406-419 did not cause any test to fail. This file restores the
 * `tests-must-fail-on-mutation-of-code-under-test-2026-04-22.md` invariant
 * for the set-password surface.
 *
 * Justification for `vi.mock` (per root CLAUDE.md test carve-out):
 *   - `getAppPool()` mock seeds a set-password-eligible row deterministically
 *     (`password_hash = NULL` AND `orcid` non-null — the two preconditions
 *     guarded at settings.ts:375 and :389) so the handler walks past every
 *     pre-argon2 guard and reaches `runWithArgon2Slot(() => argon2.hash(...))`
 *     at settings.ts:398 with no real `accounts` table writes. Carve-out
 *     clause (a): a real-HAF variant would need an ORCID-verified
 *     null-hash account plus an active session plus argon2 saturation —
 *     not deterministic per-test.
 ...
 *     queue or wiring real shutdown/abort signals.
 *   - `verifyHiveSignature` is NOT mocked. The route requires
 *     authentication; we satisfy it via a legitimate Bearer JWT signed
 *     with `config.sessionSecret`. No middleware bypass.
 *   - `redis.js` is mocked to no-redis so the in-memory rate-limiter
 *     fallback engages. A unique username per test keys the writeLimiter
 *     to a fresh bucket each call.
 ...
 * `if (err instanceof X)` lines in `backend/src/routes/settings.ts` was
 ...
 * matching assertion here. Branches verified: `ArgonQueueFullError` at
 * settings.ts:406, `ShuttingDownError` at settings.ts:410, `ArgonAbortError`
 * at settings.ts:414.
 */
...
//   1. verifyHiveSignature middleware: SELECT sessions_invalidated_at (no invalidation).
//   2. Route handler: SELECT id, password_hash, orcid (set-password-eligible row).
...
describe('POST /api/settings/set-password — argon2-semaphore error → HTTP translation', () => {
