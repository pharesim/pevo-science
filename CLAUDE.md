...
**Shared-index race discipline (multi-agent checkout).** PEvO runs architect, backend, ui, and pinner agents concurrently against one `.git`. The index is a shared mutable resource — sibling agents can stage paths between your `git status` and your `git commit`. Three rules follow from this, each enforced at a different layer:
