...
**Mechanical gate (`.githooks/pre-commit`).** A repo-local pre-commit hook fails the commit when a NEWLY-ADDED line under `frontend/{src,tests}` or `backend/{src,tests}` introduces a task-slug citation, a round/hold ordinal, an `Option X.N` label, a source line-number / `Lnn` cite, or a `tasks-archive` / `see task` / `AC #N` redirect. It is a DIFF gate (added lines only): those trees carry accepted pre-existing gray-area IDs, so a whole-tree-clean gate is **deferred** until they are swept clean. The whole-tree-clean guarantee for `backend/src/` is the standing `backend/tests/eslint/no-stale-comment-anchors.test.ts` vitest canary; this hook **complements** it — wider dirs (`backend/tests`, `frontend/**`) and wider slug families (`BE-`/`SEC-`/`JFR-`), but new-rot-only. Detection is scoped to known slug PREFIX families so legit hyphenated-uppercase tokens (`SHA-256`, `AES-GCM`, `CASE-WHEN`, the base58 `HJ-NP` class) never trip it. Activate per clone via `git config core.hooksPath .githooks` (same one-time step as the zone audit). Exempt a single legitimate fixture line (a regex self-test, an intentional stack-trace assertion) with the literal `anchor-allow` marker in a comment on that line; skip a whole commit with `PEVO_ANCHOR_GATE=off git commit ...` (never `--no-verify`). Edit the hook → re-run `bash .githooks/tests/test-pre-commit.sh`. Rationale + the deferred classes (`~<n>` tilde, bare `SEC-<n>` header IDs): `agents/docs/solutions/conventions/`.
